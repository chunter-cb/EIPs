...
Each actor occupies a single `actor_config` slot containing the authenticator address, an optional expiry, the actor's install epoch, and the scope bitmask. When `scope` includes `POLICY` (`0x02`), the actor also carries a signed policy commitment and a manager address in the separate policy slots `policy_commitment`/`policy_manager` (see [Actor Policies](#actor-policies)). Actors are revoked by deleting the `actor_config` slot. The self-actor (`actorId == bytes32(bytes20(account))`) is the one exception; its layout and rules are collected in [Self-Actor](#self-actor).

| Field | Bytes | Description |
|-------|-------|-------------|
| `authenticator` | 0–19 | Authenticator contract address |
| `expiry` | 20–25 | `uint48` Unix timestamp (seconds); the actor is invalid once `block.timestamp > expiry`. `0` = no expiry |
| `installEpoch` | 26–28 | `uint24` local-channel epoch at which the actor was authorized. Checked against the account's current epoch for non-admin actors (see [Actor Epoch](#actor-epoch)) |
| `scope` | 29–30 | `uint16` permission bitmask (`0x0000` = unrestricted; also the admin predicate, see [Actor Scope](#actor-scope)) |
| reserved | 31 | MUST be zero on write (native and EVM paths). Implicit version discriminator: new-format configs with nonzero reserved fail to apply through legacy deployments rather than applying with restrictions dropped. `scope` occupies the last populated bytes so a future widening consumes reserved space and shifts nothing |
...
Each account's local change channel carries an **epoch** in the high bits of `local_sequence` (see [Account Lock](#account-lock) for the packed layout). Every non-admin actor records the epoch at which it was authorized in its `installEpoch` field, and the protocol treats an actor as **epoch-live** only while its `installEpoch` equals the account's current epoch:

- **Non-admin actors** (`scope != 0`) are checked at authentication: if `installEpoch != currentEpoch`, the actor is rejected (`ActorEpochRevoked`). This is one cold SLOAD of the account-state slot, paid only by restricted actors.
- **Admin actors** (`scope == 0`) are exempt. This is required, not a convenience: bumping the epoch is itself a `scope == 0` signed operation, so subjecting admins to the epoch check would destroy the very key needed to authorize recovery. The inline secp256k1 self is exempt by construction (its inline config has no room for a stamp; `DEFAULT_EOA_REVOKED` revokes it individually).

`installEpoch` is stamped from the epoch **bound in the signed change's digest**, never the current epoch at apply time. Stamping the live epoch would let a `[bump, replay]` sequence resurrect a change with a fresh stamp; binding the epoch in the signature makes a change apply only against the epoch its signer intended.

Bumping the epoch (see [Epoch Bump](#epoch-bump)) therefore revokes every non-admin actor at the old epoch instantly, at authentication, with no enumeration and node-filterable. Durable revocation of a single actor without a bump is available through the actor's own `expiry` or an explicit `revokeActor`; the epoch bump is the mass, all-at-once brake.
...
| `local_sequence` | Local-chain change channel (`uint64`), reinterpreted as a packed epoch + nonce: bits `[63:40]` are the `epoch` (`uint24`), bits `[39:0]` are the `nonce` (`uint40`). `local_sequence != 0` still doubles as the initialized flag (bootstrap sets `nonce = 1`, and any epoch bump leaves the epoch bits non-zero). The field offset and width are unchanged, so node raw-slot parsing for lock tiering is unaffected; `multichain_sequence` is untouched. See [Actor Epoch](#actor-epoch) and [Epoch Bump](#epoch-bump) |
...
The local channel splits into an epoch and a nonce so that the epoch acts as a mass-revocation generation counter while the nonce orders sequenced changes within an epoch:

```solidity
uint256 internal constant EPOCH_SHIFT   = 40;
uint64  internal constant SEQUENCE_MASK = (uint64(1) << 40) - 1;   // low-bits mask; also the nonce MAX sentinel

function _epochOf(uint64 w) internal pure returns (uint24) { return uint24(w >> EPOCH_SHIFT); }
function _nonceOf(uint64 w) internal pure returns (uint64) { return w & SEQUENCE_MASK; }
```

A `nonce` value of `SEQUENCE_MASK` is the **MAX sentinel**: an unordered, non-consuming local-change slot used to authorize actors independently (see [Config Change Authorization](#config-change-authorization)). `multichain_sequence` is not split; it remains a plain `uint64` counter.
...
Lock operates using the local account channel. The digest binds `chainId = block.chainid` and `sequence = local_sequence` (the **current** packed epoch+nonce value); the contract rejects a mismatch and, on success, increments the `nonce` portion — the same sign-current-then-increment convention as local config changes (which share this counter). `applySignedLockChanges` rejects the MAX sentinel (`_nonceOf(sequence) == SEQUENCE_MASK`) outright: lock is an ordered, consuming operation. `auth` is a standard `authenticator \|\| data` blob (see [Signature Format](#signature-format)) validated as **admin** (`scope == 0x00`) against `account`. Anyone may relay; authorization comes from the signature.
...
ACTOR_TYPEHASH =
  keccak256("Actor(bytes32 actorId,ActorConfig config,bytes policyData)"
            "ActorConfig(address authenticator,uint48 expiry,uint24 installEpoch,uint16 scope)")
...
// and installEpoch is always 0 (import is the bootstrap at epoch 0; both are hashed as 0).
// policyData follows authorizeActor's rule: manager (20) || commitment (32) when POLICY is
// set, empty otherwise, and is hashed for real:
configHash_i = keccak256(abi.encode(ACTORCONFIG_TYPEHASH, authenticator_i, 0, 0, scope_i))
...
Each initial actor carries an `actorId`, `authenticator`, `scope` (`uint16`; `0x00` = unrestricted admin), and `policyData` (empty, or `manager ‖ commitment` when `POLICY` is set), all committed to the derived address (see [Address Derivation](#address-derivation)) so the counterfactual address binds each initial actor's authority. Three things are **not** expressible here: `expiry`, `installEpoch` (creation is the bootstrap at epoch 0, so every initial actor is stamped `installEpoch = 0`), and a self-referential `manager = account` (the account address is not yet known at commitment time). Both are added post-creation with a config change entry, which MAY accompany the create entry in the same `account_changes` array for atomic setup and, running after creation, resolves `manager = account` to a concrete address. For example, a subaccount is created with an admin actor (a delegate to the primary account, `scope = 0x00`) and a restricted app key (`SENDER | SELF_PAYER`, or `POLICY` with an external `manager` inline, or `manager = account` via an accompanying config change).
...
The per-actor contribution is `actorId || authenticator || scope || policyData`: the 32-byte `actorId`, 20-byte `authenticator`, 2-byte `scope`, and the `policyData` bytes (empty when `POLICY` is unset, or exactly 52 bytes — `manager (20) ‖ commitment (32)` — when `POLICY` is set). The per-actor length is fully determined by `scope`, so the concatenation remains unambiguous. Expiry and `installEpoch` do not participate (both are `0` at create). The required strictly-ascending `actorId` ordering makes the commitment canonical.
...
rlp([
  ...
  sequence,           // uint64: for chain_id 0, a monotonic multichain counter; for the local
                      //   channel, the packed epoch|nonce (see Config Change Authorization). A
                      //   nonce of SEQUENCE_MASK is the MAX sentinel (unordered, non-consuming)
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Authorize a new actor. Writes `actor_config` with `authenticator`, `scope`, and `expiry` (`0` = no expiry) verbatim (unknown scope bits stored as-is). `installEpoch` is **not** a caller input: it is stamped from the epoch bound in the change's `sequence`/digest (never the current epoch at apply time; see [Actor Epoch](#actor-epoch)). If `POLICY` is set, requires `policyData = manager ‖ commitment` (exactly 52 bytes; neither field need be nonzero) and writes those slots; otherwise requires empty `policyData` and clears policy slots. Writes the packed `actor_config` word with its reserved byte zeroed. Does **not** enforce scope-combination exclusivity (that is use-time / protocol). Emits `ActorAuthorized` whose `actorData` is 32 bytes (`authenticator ‖ expiry ‖ installEpoch ‖ scope ‖ reserved`) when `POLICY` is unset, or 84 bytes (appending `manager ‖ commitment`) when `POLICY` is set. |
...
The sequence number is scoped by `chain_id`: `0` uses the multichain sequence channel (a plain monotonic `uint64`, valid on any chain), while a specific `chain_id` uses that chain's local channel, which is the packed `epoch|nonce` word (see [Account Lock](#account-lock)). `applySignedActorChanges` takes the `sequence` explicitly and dispatches on its nonce portion:

| `_nonceOf(sequence)` | Epoch | Nonce | Consumed |
|----------------------|-------|-------|----------|
| `< SEQUENCE_MASK` (sequenced) | must equal current epoch | must equal current nonce | yes — increments the nonce |
| `== SEQUENCE_MASK` (MAX sentinel) | must equal current epoch | — (ignored) | no |

```solidity
function applySignedActorChanges(
    address account, uint256 chainId, uint64 sequence,
    ActorChange[] calldata actorChanges, bytes calldata auth
) external onlyUnlocked(account);
```

The **sequenced** path is the ordered channel: it binds the exact current `epoch|nonce`, consumes the nonce, and serializes admin changes. A **carry guard** applies: the contract reverts `SequenceExhausted` if consuming the nonce would reach `SEQUENCE_MASK - 1`, so the ordered space never collides with the MAX sentinel.

The **MAX sentinel** path lets independent actors be authorized without sharing an ordered sequence, so a fleet of session keys can be installed in any order without contention. Because it consumes no nonce, it is tightly restricted: `chainId != 0` (local channel only), `AUTHORIZE_ACTOR` operations only, each authorized actor MUST have `scope != 0` (no admin via this path) and `expiry != 0` (durable revocation without a bump), the change MUST be insert-only (it MUST NOT overwrite an existing actor), and the signer MUST be admin (`scope == 0`). `expiry != 0` is what keeps routine per-agent revocation independent of the epoch brake: a single agent key can lapse or be revoked without invalidating the fleet.

The change-sequence channels double as the initialized flag. Creation and import set the local channel to `1` (epoch `0`, nonce `1`), and any applied sequenced change bumps the relevant channel (the local nonce for a chain-specific `chain_id`, the multichain counter for `chain_id 0`). Otherwise only the implicit EOA is on the account.
...
`applySignedEpochBump` advances the local channel's epoch, which mass-revokes every non-admin actor authorized at the old epoch (see [Actor Epoch](#actor-epoch)). It is a dedicated admin-authorized EVM entry point:
...
The digest binds `chainId = block.chainid`. The contract requires `epoch == _epochOf(local_sequence)` (the current epoch), a signer that is admin (`scope == 0`), and `epoch < type(uint24).max` (reverts `EpochExhausted` otherwise; `EpochMismatch` on a stale epoch). On success it sets `local_sequence = uint64(epoch + 1) << EPOCH_SHIFT` — advancing the epoch and resetting the nonce to `0` — and emits `EpochBumped(account, epoch + 1)`. Anyone may relay.

The bump is **epoch-bound, not nonce-bound**, so it never races a pending admin change for a particular nonce slot, and it is self-invalidating: a replay of a bump binding epoch `E` fails once the epoch is `E+1`. It consumes no nonce. Unlike the other config paths it is **not** gated by `onlyUnlocked`: a bump is purely destructive (it can never grant authority), and refusing it while locked would create a brick path if a replayable lock ever landed.

**Effect of one bump.** In a single operation it: (1) kills every pending local signed change — both sequenced (the nonce reset invalidates their bound `epoch|nonce`) and MAX-sentinel (their bound epoch is now stale); and (2) revokes every non-admin actor stamped at the old epoch, instantly at authentication, node-filterable, with no enumeration. Sequencing `[revokeActor X, bump]` makes `X` permanently unresurrectable. The bump is all-or-nothing for live non-admin actors: recovering after a bump costs re-authorizing the fleet, which is exactly why MAX-path actors carry `expiry != 0` — routine per-agent revocation stays independent of this mass brake.
...
| `EPOCH_SHIFT` | `40` | Bit offset of the `epoch` within the packed local channel (`local_sequence`); the `nonce` occupies bits `[39:0]` (see [Account Lock](#account-lock)) |
| `SEQUENCE_MASK` | `(1 << 40) - 1` | Low-40-bit nonce mask; also the local-channel **MAX sentinel** nonce (unordered, non-consuming config change; see [Config Change Authorization](#config-change-authorization)) |
...
Two goals pull in opposite directions: routine automation wants many session keys installed and revoked independently, while a compromised account wants one atomic "revoke everything" brake. A single ordered sequence serves neither well — sharing a sequence across session keys forces contention and total ordering on installs, and per-key revocation via that sequence cannot express "kill the whole fleet at once."

Splitting the local channel into an `epoch` and a `nonce` resolves both. The **MAX-sentinel** config path installs each non-admin actor without consuming the ordered nonce, so a fleet of agent keys can be authorized in any order with no contention; their durability comes from their own `expiry`, keeping routine per-agent revocation independent of anything else. The **epoch** is a generation counter stamped into each non-admin actor's `installEpoch`; a single [epoch bump](#epoch-bump) advances it and revokes every non-admin actor at the old generation instantly at authentication — no enumeration, node-filterable — while admins (the recovery root) are exempt by construction. Stamping the epoch bound in the signature rather than the live epoch prevents a `[bump, replay]` from resurrecting a change with a fresh generation. The result: cheap, unordered, individually-expiring session keys for the common case, plus an all-or-nothing kill switch for the compromise case, sharing one 64-bit slot and adding at most one cold SLOAD to restricted-actor authentication.
...
The `actor_config` layout (`authenticator ‖ expiry ‖ installEpoch ‖ scope ‖ reserved`, with the scope-bit assignment and the reserved byte acting as a version gate), the local-channel `epoch|nonce` split, the address-derivation commitment, and the ABI surface (typehashes, events, `importAccount`/`applySignedActorChanges`/`applySignedEpochBump` signatures) are defined by this specification. Because 8130 has no prior deployment, there is no live actor state written under an earlier format to migrate; the formats here are authoritative from first deployment.
...
interface IAccountConfiguration {
    struct ChangeSequences {
        ...
        uint64 local;      // chain_id == block.chainid; packed epoch|nonce (epoch = bits[63:40], nonce = bits[39:0]).
                           // starts at epoch 0, nonce 1 once initialized (created/imported); 0 = uninitialized
    }

    struct ActorConfig {
        address authenticator;
        uint48 expiry;       // Unix seconds; 0 = no expiry. Actor invalid once block.timestamp > expiry
        uint24 installEpoch; // local-channel epoch at authorization; non-admin actors are rejected once it != current epoch
        uint16 scope;        // grants bitmask; 0x00 = unrestricted (admin); 0x01 = SENDER; 0x02 = POLICY; 0x04 = NONCE; 0x08 = SELF_PAYER; 0x10 = SPONSOR_PAYER; ERC-1271 signing requires operational authority (admin or SENDER without POLICY), not a grant
    }
    ...
    // expiry and installEpoch are NOT expressible here (initial actors are non-expiring and stamped at epoch 0);
    // expiry is added post-deployment via config changes.
    struct InitialActor {
        bytes32 actorId;
        address authenticator;
    ...
    //   authenticator(20) || expiry(6) || installEpoch(3) || scope(2) || reserved(1) — 32 bytes —
    ...
    error ActorEpochRevoked();  // non-admin actor's installEpoch != current epoch
    error EpochMismatch();      // bump bound to a stale epoch
    error EpochExhausted();     // epoch already at type(uint24).max
    error SequenceExhausted();  // ordered nonce would reach SEQUENCE_MASK - 1
    ...
    // sequence: for chain_id 0, the multichain counter; for the local channel, the packed epoch|nonce.
    // A nonce of SEQUENCE_MASK is the unordered, non-consuming MAX sentinel (see Config Change Authorization).
    function applySignedActorChanges(address account, uint256 chainId, uint64 sequence, ActorChange[] calldata actorChanges, bytes calldata auth) external;

    // Advance the local epoch, mass-revoking non-admin actors at the old epoch. Admin-authorized, relayable.
    // Binds block.chainid; requires epoch == current epoch and epoch < type(uint24).max. Consumes no nonce; not gated by lock.
    function applySignedEpochBump(address account, uint24 epoch, bytes calldata auth) external;

    // Account lock. Signed, relayable, admin-authorized. op: 1 = lock, 2 = unlock.
    // Local channel only: binds block.chainid and the current local_sequence, incrementing the nonce; rejects the MAX sentinel.
    ...
    function authenticateActor(address account, bytes32 hash, bytes calldata auth)
        external view returns (uint16 scope, address policyTarget);
...
**Actor Epoch**: The epoch check only revokes **non-admin** actors (`scope != 0`); admins are exempt so the `scope == 0` signer needed to bump or recover always survives. Consequently an epoch bump is not a substitute for revoking a compromised admin — a rogue admin must be removed with `revokeActor` (and the account SHOULD keep a separate non-expiring admin so a `[revoke, bump]` recovery chain can be signed). `installEpoch` MUST be stamped from the epoch bound in the change's signature, never the live epoch at apply time; stamping live would let `[bump, replay]` resurrect a change at the new generation. Because one bump invalidates every pending local change (sequenced and MAX-sentinel) and every non-admin actor at the old epoch, wallets SHOULD treat it as a fleet-wide reset and rely on per-actor `expiry`/`revokeActor` for routine, isolated revocation. The bump is intentionally not gated by lock: it can only remove authority, and gating it would create a brick path.
