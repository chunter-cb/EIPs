...
The protocol validates signatures by reading `actor_config` directly and delegating authentication to [Authenticators](#authenticators) — see [Validation](#validation) for the full flow. Actor enumeration is performed off-chain via `ActorAuthorized`, `ActorReauthorized`, and `ActorRevoked` event logs; for any `actorId` the most recent of these three reflects the actor's current authorization. 
...
| `0x03` | `reauthorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Replace the configuration of an **existing** actor in place. Reverts if the `actorId` is not currently configured. Clears the actor's `policy_commitment`/`policy_manager` slots, then applies the new `config` and `policyData` exactly as `authorizeActor` does (same `policyData` slicing, the same `len(policyData)` check, and the same rejection of a policy-bearing actor whose `scope` is unrestricted or includes CONFIG). For the self-actor it updates whichever home the actor occupies (inline default-EOA fields for a `K1_AUTHENTICATOR` self, or the `actor_config(self)` slot for a non-secp256k1 self) and preserves the inline/non-k1 mutual exclusion; the `authorizeActor` self re-authorization guard does **not** apply, since replacement is the explicit intent. Emits `ActorReauthorized`. |

`authorizeActor` and `reauthorizeActor` are deliberately split by intent: the former is create-only and reverts on an already-configured `actorId` (so a client cannot silently clobber an existing credential), while the latter is the explicit in-place replace. Reconfiguring an existing actor (e.g. narrowing a key's `scope` or refreshing its `expiry`) is therefore a single `reauthorizeActor` operation — one `actor_config` reset write (cold SLOAD + warm SSTORE reset, ~5,000 gas) — rather than a `revokeActor` + `authorizeActor` pair, which consumes two of the per-transaction config-change slots and is priced as a clear plus a fresh `SSTORE_SET` (~27,000 gas) under the [intrinsic gas schedule](#intrinsic-gas), which does not refund the intermediate clear. The end state of a `reauthorizeActor` is identical to that revoke-then-authorize pair; only the cost, the slot count, and the emitted event differ.
...
### Why a Separate `reauthorizeActor`?

Reconfiguring an existing actor (narrowing a `scope`, refreshing an `expiry`, rotating an authenticator) is a common lifecycle operation, but `authorizeActor` is intentionally create-only: it reverts on an already-configured `actorId` so that a client cannot silently overwrite a live credential by reusing an id. Expressing a reconfiguration as `revokeActor` + `authorizeActor` reaches the correct end state but is unnecessarily expensive — it consumes two of the bounded per-transaction config-change slots and, under the non-refunding intrinsic gas schedule, is priced as a slot clear plus a fresh `SSTORE_SET` (~27,000 gas) rather than a single `SSTORE_RESET` (~5,000 gas).

`reauthorizeActor` (`0x03`) is the explicit in-place replace: it requires the actor to already exist, resets the policy slots, and rewrites `actor_config` once. Splitting it from `authorizeActor` by intent keeps the create path strictly fail-loud (an accidental id reuse still reverts) while making the replace path unambiguous in the signed calldata and cheap. The alternative — making `authorizeActor` an upsert — was rejected because it removes the fail-loud guard and makes "create" and "silently replace" indistinguishable at the call site. Observability is unaffected: the most recent of `ActorAuthorized` / `ActorReauthorized` / `ActorRevoked` for an `actorId` reflects its current state.
...
interface IAccountConfiguration {
    ...
    struct ActorChange {
        uint8 changeType;  // 0x01 = authorizeActor, 0x02 = revokeActor, 0x03 = reauthorizeActor
        bytes32 actorId;
    ...
    event ActorReauthorized(address indexed account, bytes32 indexed actorId, ActorConfig config, address policyManager, bytes32 policyCommitment);
