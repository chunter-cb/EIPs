...
4. **Verify**: K1 is self-authenticating (recovery is the proof). P256 verifies via RIP-7212. Explicit: reject if the verifier address has an [EIP-7702](./eip-7702.md) delegation designator (`0xef0100`); then execute `verify(account, keyId, hash, data)` on the verifier in the sandbox.
...
```
operationHashes = [keccak256(abi.encode(opType, verifier, keyId, flags)) for each operation]
...
```
operationHashes = [keccak256(abi.encode(opType, flags, unlockDelay)) for each operation]
operationsHash = keccak256(abi.encodePacked(operationHashes))
...
Nodes MAY restrict mempool acceptance to transactions using native verifiers or a node-configured allowlist of known sandbox verifiers. With this gating, validation requires only Account Configuration state (`key_config`) and nonce state — no contract execution during validation. Once a transaction validates, only two state changes can invalidate it: key revocation (`key_config` set to `address(0)`) or nonce consumption — a single SLOAD confirms continued validity.
...
**Deterministic Validation**: Validation requires only state reads (`key_config`, nonce, balance) and pure-function execution — native implementations for recognized verifiers, sandboxed contracts with node-enforced gas caps for others (see [Sandbox Verifiers](#sandbox-verifiers)). Results are fully determined by current state, making validation easily cacheable and parallelizable. This eliminates invalidation-based DoS without requiring a reputation system or tracing. Once a transaction validates, the invalidation surface is minimal: only `key_config` changes (key revocation) or nonce consumption can invalidate it. Nodes gating on native and known verifiers need only Account Configuration state and nonce state, enabling lightweight edge validation with no contract execution.
