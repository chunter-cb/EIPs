...
This specification defines the protocol-level (consensus) behavior: the transaction type, validation and execution, intrinsic gas, and the authenticator model. The **Account Configuration Contract is the canonical reference for its own internals** — exact storage slot packing, ABI, EIP-712 typehashes, event encodings, and EVM-only entry points — maintained in the [companion contract repository](https://github.com/8130-working-group/contracts). An implementation MAY either call the deployed contract or replicate its logic as pure protocol-level code with static/calculated gas; either way the contract source is authoritative for encodings and slot layout. Where a detail is consensus-critical (read or verified natively on 8130 chains), this document states the normative summary and links the repository for the full definition.
...
**Registration.** **A** authorizes an actor with `authenticator = DELEGATE_AUTHENTICATOR` and `actorId = bytes32(bytes20(B))`. That actor's `scope` in **A**'s config governs what **B** may do for **A** under the normal [Actor Scope](#actor-scope) rules. Registering and revoking delegate actors requires admin (`scope == 0x00`) or an actor holding the `DELEGATE` grant (`0x20`); a `DELEGATE`-only actor is confined to managing delegate actors and cannot touch any non-delegate actor (see [Actor Scope](#actor-scope)).
...
Each actor occupies a single `actor_config` slot keyed by `(account, actorId)` holding the actor's `authenticator` (address), `scope` (`uint16` permission bitmask; `0x0000` = unrestricted, see [Actor Scope](#actor-scope)), and `expiry` (`uint48` Unix seconds; `0` = no expiry, invalid once `block.timestamp > expiry`), with the remaining bytes reserved and MUST be zero — the nonzero-reserved check acts as a version gate. Validity is decided by this single SLOAD. When `scope` includes `POLICY` (`0x02`), the actor additionally carries a signed `commitment` (`bytes32`) and a `manager` address in separate policy slots keyed by `(account, actorId)`, read only during execution (see [Actor Policies](#actor-policies)). Actors are revoked by clearing the slot. The self-actor (`actorId == bytes32(bytes20(account))`) is the one exception; see [Self-Actor](#self-actor).

The exact byte packing of these slots is defined by the deployed contract and is canonical in the [contract repository](https://github.com/8130-working-group/contracts); the protocol reads the same slots directly.

#### Self-Actor

The **self-actor** is the actor whose `actorId == bytes32(bytes20(account))`. It exists in exactly one of two mutually exclusive forms, and registering one clears the other:

1. **Inline secp256k1 self (default EOA).** Held inline in the packed account-state slot (`scope`/`expiry` plus the `DEFAULT_EOA_REVOKED` flag; see [Account Lock](#account-lock)), authenticated via native ecrecover. This is what makes an EOA usable immediately: a native secp256k1 signature recovering to the account authenticates as the self-actor whenever `DEFAULT_EOA_REVOKED` is unset. For a fresh account the inline config is all-zero (`scope == 0x00`, `expiry == 0`) — a non-expiring admin. Registering the self with `K1_AUTHENTICATOR` (`address(1)`) writes a custom `scope`/`expiry` inline while retaining native ecrecover; setting `DEFAULT_EOA_REVOKED` disables this form entirely. (Scoping restriction: [Implicit EOA Rule Scoping](#security-considerations).)
2. **Non-secp256k1 self authenticator.** An authenticator that returns the self-actorId, held in the `actor_config(self)` slot. Registering it sets `DEFAULT_EOA_REVOKED`, disabling the inline k1 self.

Account creation and import set `DEFAULT_EOA_REVOKED` by default, so a new or imported account leaves no native secp256k1 owner live unless one is among its initial actors (a quantum-safe default).

#### Actor Scope

The scope field in `actor_config` is a `uint16` permission bitmask of **grants**. A value of `0x0000` means unrestricted (all contexts) and is also the account's **admin** predicate: any check in this specification that requires an "admin" actor is exactly `scope == 0x00`. Any non-zero value grants only the contexts whose bits are set. Reads are fail-closed: a context is authorized only when `scope == 0x00` or the corresponding grant bit is present. Unknown bits grant nothing. All future scope bits MUST be pure grants. An actor whose scope sets only unknown bits is stored verbatim but authorizes no context — a permanently inert actor until a protocol change defines those bits.
...
| 5 | `0x20` | DELEGATE | Manage delegate actors: authorizes authorizing/revoking actors whose `authenticator == DELEGATE_AUTHENTICATOR` (control over which other accounts may act on this account's behalf), without conferring full admin config authority (see [Delegate Authenticator](#delegate-authenticator)) |
| 6–15 | | (spare) | Reserved for future pure grants |

**Admin.** Config-change authority (`authorizeActor`, `revokeActor`, `applySignedActorChanges`, delegation) is exactly the admin predicate `scope == 0x00`: the same unrestricted root every other check already special-cases. Accounts are born with a scope-0 root (the implicit EOA, or an unrestricted actor named at create/import); everything below that root is granted. The one carve-out is `DELEGATE`: an actor holding `DELEGATE` (but not admin) may authorize and revoke actors whose `authenticator == DELEGATE_AUTHENTICATOR` and no others, so an account can delegate control over its delegate set without handing out full admin. See [Why Admin Is Scope Zero](#why-admin-is-scope-zero).
...
| `default_eoa_scope` | Inline self-actor `scope` (`uint16`; `0x0000` = full owner) |
...
Lock state changes only through `applySignedLockChanges`, a dedicated admin-authorized (`scope == 0x00`) EVM-only entry point on the local account channel (sign-current-then-increment `local_sequence`, shared with transaction nonces and local config changes). Anyone may relay; authorization comes from the signature.

**Lifecycle** — `lock`, then `unlock`, with no other actions in between:

1. **Lock**: only from the unlocked state. Sets `LOCKED` and stores `unlock_delay`. Rejected if already locked; the delay cannot be changed while locked.
2. **Unlock**: only from `LOCKED` with no pending unlock. Sets `UNLOCK_INITIATED` and `unlocks_at = block.timestamp + unlock_delay` (from the stored delay).
...
The `SignedLockChange` typehash and the exact `applySignedLockChanges` signature are canonical in the [contract repository](https://github.com/8130-working-group/contracts).

#### Account Import

`importAccount` is an EVM-only entry point (not on the 8130 transaction path) that registers an already-deployed account (e.g. an existing ERC-4337 wallet) into the Account Configuration Contract with an initial actor set. It is a one-time bootstrap: it is rejected unless both change-sequence channels are empty (a locked account is already initialized and so is caught here). The initial actor set is authorized by the account's existing [ERC-1271](./eip-1271.md) `isValidSignature` over a typed digest bound to the account address and a replay-domain `chainId` (`0` = multichain, else `block.chainid`, mirroring `applySignedActorChanges`). Imported actors are always non-expiring (expiry is added post-import via config changes); `manager = account` is expressible here. On success `importAccount` sets `DEFAULT_EOA_REVOKED` (parity with account creation); an owner keeps their native key by including the self-actorId as a `K1_AUTHENTICATOR` entry in the initial actors.

The exact `ActorInitialization` typehash and digest construction are defined by the deployed contract and are canonical in the [contract repository](https://github.com/8130-working-group/contracts).
...
`DEFAULT_ACCOUNT_ADDRESS` SHOULD implement token receiver hooks (ERC-721, ERC-1155) to safely receive assets. [ERC-1271](./eip-1271.md) signature validation is wallet-defined and not provided by the protocol.
...
| `expiry` | `uint64` Unix timestamp (milliseconds since epoch); encoded as a canonical minimal-length RLP integer. Transaction invalid when `block.timestamp * 1000 > expiry`. A value of `0` means no expiry. Must be non-zero when `nonce_key == NONCE_KEY_MAX`. |
...
Config change entries manage the account's actors. Each entry carries a `chain_id` (`0` = valid on any chain, for cross-chain sync; else a specific chain), a monotonic `sequence`, an array of actor changes, and an `auth` signature valid at that sequence. Each actor change is a `(change_type, actorId, data)` tuple where `data` is an opaque, ABI-encoded blob decoded identically whether applied natively (8130 path) or via `applySignedActorChanges()` (EVM); it is hashed as `keccak256(data)` in the signature payload.

**Operation types**:

| change_type | Name | Effect |
|-------------|------|--------|
| `0x01` | `authorizeActor` | Authorize/overwrite an actor: writes `authenticator`, `scope`, and `expiry` verbatim (unknown scope bits stored as-is; reserved bytes zeroed). If `POLICY` is set, `data` must carry `manager ‖ commitment` and the policy slots are written; otherwise policy slots are cleared. Scope-combination exclusivity is not enforced at write time (checked at use). Emits `ActorAuthorized`. |
| `0x02` | `revokeActor` | Revoke an actor: clears `actor_config` and its policy slots. For the implicit EOA actor (`actorId == bytes32(bytes20(account))`), instead sets `DEFAULT_EOA_REVOKED`. Emits `ActorRevoked`. |

The exact RLP/ABI encoding of the entry, the `data` layouts, and the `ActorAuthorized` `actorData` layout are canonical in the [contract repository](https://github.com/8130-working-group/contracts).
...
Entry signatures are over a `SignedActorChanges` typed digest binding `account`, `chainId`, `sequence`, and the (individually hashed) actor changes. On 8130 chains the protocol verifies this digest natively, so it is consensus-critical; the exact typehash and digest construction are canonical in the [contract repository](https://github.com/8130-working-group/contracts). Domain separation from transaction signatures (`AA_TX_TYPE`, `AA_PAYER_TYPE`) is structural: transaction hashes use `keccak256(type_byte || rlp([...]))`, which cannot collide with the `abi.encode(TYPEHASH, ...)` digest. The `auth` follows the same [Signature Format](#signature-format) as `sender_auth` (`authenticator || data`), validated against the account's actor state at that point in the sequence.
...
Calls carry no ETH value. ETH transfers are initiated by the account's wallet bytecode via the CALL opcode; since every account has wallet bytecode (via auto-delegation or explicit deployment), no capability is lost and the protocol never moves ETH on the sender's behalf.

##### Call Phases

`calls` is a two-level structure: an ordered array of **phases**, where each phase is an ordered array of individual calls (`[[call, ...], [call, ...]]`). This gives two levels of atomicity where calls grouped within a phase are all-or-nothing, while phases commit independently in sequence. This supports atomic batching (one phase) and sponsor protection (payment in phase 0 persists even if user actions in phase 1 revert) without per-call mode flags.
...
`DEPLOYMENT_HEADER(n)` is a fixed 14-byte EVM loader that copies the trailing `n` bytes of runtime code into memory and returns it (the standard CODECOPY/RETURN init prologue, with `n` encoded into its `PUSH2` instructions). It participates in [Address Derivation](#address-derivation) via `deployment_code = DEPLOYMENT_HEADER(len(code)) || code`, so its exact bytes are consensus-critical and identical on 8130 and non-8130 chains; the canonical byte sequence is defined in the [contract repository](https://github.com/8130-working-group/contracts). The create entry only supports runtime bytecode; delegation is set via delegation entries (type `0x02`) in `account_changes`.
...
Additional `nonce_key` values allow parallel transaction lanes without nonce contention between independent workflows. `NONCE_KEY_MAX` enables nonce-free transactions where replay protection comes from short-lived `expiry` and node-level deduplication by the fee- and signature-invariant [Replay Identifier](#replay-identifier), useful for operations where nonce ordering coordination is undesirable. Reserving `NONCE_KEY_MAX` as the sentinel permanently removes one channel from the `2^256`-wide nonce space, which is immaterial in practice. Making sequenced channels a `NONCE` grant rather than the default keeps the common session-key case coordination-free (a restricted actor without `NONCE` is confined to nonce-free `NONCE_KEY_MAX`) and reserves ordered channels for actors that explicitly want them.
...
The `actor_config` layout (`authenticator ‖ expiry ‖ scope ‖ reserved`, with the scope-bit assignment and the reserved bytes acting as a version gate), the address-derivation commitment, and the ABI surface (typehashes, events, `importAccount`/`applySignedActorChanges` signatures) are defined by this specification. Because 8130 has no prior deployment, there is no live actor state written under an earlier format to migrate; the formats here are authoritative from first deployment.

## Reference Implementation

### IAccountConfiguration

The deployed Account Configuration Contract is the **canonical ABI surface**. The full interface — the account-mutating entry points (`createAccount`, `computeAddress`, `importAccount`, `applySignedActorChanges`, `applySignedLockChanges`), their structs (`InitialActor`, `Actor`, `ActorChange`, `ChangeSequences`), and events — is maintained in the [contract repository](https://github.com/8130-working-group/contracts). The subset below is the read/authentication surface that the protocol and wallets rely on and that other parts of this specification reference.
...
interface IAccountConfiguration {
    struct ActorConfig {
        ...
        uint16 scope;       // grants bitmask; 0x0000 = unrestricted (admin); 0x01 = SENDER; 0x02 = POLICY;
                            // 0x04 = NONCE; 0x08 = SELF_PAYER; 0x10 = SPONSOR_PAYER; 0x20 = DELEGATE
    }

    // Protocol-injected receipt logs on 8130 chains (also emitted by the EVM entry points).
    // Full event signatures are canonical in the contract repository.
    ...
    event DelegationApplied(address indexed account, address target); // 8130 path only
    ...
    // Actor authentication: returns the actor's authorization surface verbatim; reverts on failure.
    ...
    // Read views (verbatim reporting). getPolicy returns (manager, commitment); zero when POLICY unset.
...
The full transaction hash MUST NOT be used for nonce-free deduplication or mempool replacement. The hash commits to the fee fields and the `sender_auth`/`payer_auth` blobs, which `replay_id` excludes; keying on it would let a fee bump, a re-signed `payer_auth`, or non-deterministic/malleable `sender_auth` masquerade as unrelated transactions or trivially duplicate one logical transaction. All such variants resolve to the same `replay_id`, collapsing them to a single mempool slot and a single includable transaction. Because `expiry` *is* part of `replay_id`, it cannot be extended via a fee-bump replacement: a new `expiry` is a new logical transaction that must independently satisfy the acceptance rules.
