...
Aggregation is **only** an optimization of signature verification, and it requires **no change to the [AA transaction type](#aa-transaction-type)**. Because 8130 already routes every authentication through an authenticator, an aggregatable signature is expressed entirely within the existing `sender_auth`, `payer_auth`, or config-change `auth` blob — the same place a non-aggregated signature goes. Concretely, aggregation adds only: a class of sentinel authenticator, and one block-level validity rule (the recursive STARK). Everything else — the wire format, authorization, scope, nonce, policy, ordering — is unchanged.

A block dependency is a `(scheme, data_hash, verification_key)` triple, derived by the protocol from an aggregatable authenticator used in `sender_auth`, `payer_auth`, or a config-change `auth`; nothing extra is carried in the transaction (see [Aggregatable Authenticators](#aggregatable-authenticators)). Signatures verified *inside* the EVM (e.g. [ERC-1271](./eip-1271.md)) are not aggregated and continue to verify inline (see [ERC-1271 and In-EVM Signatures](#erc-1271-and-in-evm-signatures)).
...
Dependencies are not carried in the transaction. The protocol materializes each triple from the aggregatable authenticator's auth blob: `data_hash` is the relevant signature hash (see [What Aggregation Defers](#what-aggregation-defers)) and `verification_key` is the auth blob's key. A transaction's dependency set is one triple per aggregatable auth blob it uses (`sender_auth`, `payer_auth`, and/or config-change `auth`); the block's set is the concatenation across all transactions in block order.
...
The block is valid only if `block_deps_hash` equals the hash of the dependencies collected from all AA transactions (in block order) and `stark_proof` verifies against `block_deps_hash` and the protocol-level `AGGREGATED_VK`. The proof uses the "verify, merge, and discard" recursive construction of [EIP-8288](./eip-8288.md). This proposal adapts that construction: EIP-8288 declares dependencies through [EIP-8141](./eip-8141.md) frames, whereas here the authenticator already names the actor, so the auth blob is the declaration site and no frame layer — nor any transaction-format change — is required.
...
2. **Dependency set tracks the included set.** `block_deps_hash` MUST contain the dependencies of exactly the included transactions — no more, no less. When a transaction is dropped, its dependencies MUST be removed from the set (the recursive construction's *discard* mechanism), so the proof does not attest a signature for a transaction that is not in the block.
...
Aggregation covers transaction authentication only. Signatures verified *inside* the EVM — most importantly [ERC-1271](./eip-1271.md) `verifySignature()`, but also any contract that checks a signature — are **not** aggregated: the full signature is passed to the verifier and checked on the spot, so a post-quantum signature pays its full inline cost (~150,000 gas for leanSPHINCS).

Aggregating an in-EVM check would require declaring its dependency ahead of execution (the algorithm cannot see, before running, which signatures a call will check), which in turn would require carrying that declaration in the transaction. This proposal deliberately does not change the [AA transaction type](#aa-transaction-type), so no such declaration exists and in-EVM verification stays inline. This keeps aggregation to the one case the protocol already sees up front — the authenticator that authorizes the transaction — and preserves off-chain `eth_call` behavior, where there is no block proof to consult.

#### Mempool

A node cannot verify an aggregatable signature from the transaction alone, since the block-form auth blob carries only the `verification_key`; the signature (witness) travels separately. Such transactions are gossiped inside an [EIP-8288](./eip-8288.md) **wrapper object** that bundles transactions with either their individual proofs or a recursive STARK, and that supports recursive re-aggregation between peers. A node MUST hold a valid proof for every dependency before accepting the transaction, and SHOULD enforce the per-transaction limits `MAX_SIGS_PER_TX` and `MAX_STARKS_PER_TX`. All non-cryptographic mempool checks ([Mempool Acceptance](#mempool-acceptance)) are unchanged.
...
**Why the authenticator seam rather than frames.** EIP-8288 layers on [EIP-8141](./eip-8141.md) and needs a dedicated frame mode to *declare* which signatures to prove. 8130 already routes every authentication through an authenticator that names the actor, so transaction authentication needs no new declaration surface and **no change to the transaction type** — the existing auth blob is the declaration. The cost is that only transaction authentication is aggregatable; in-EVM checks (e.g. ERC-1271), which the protocol cannot see ahead of time, would require a transaction-level declaration and so remain inline. This is a deliberate trade to keep the transaction format fixed and the common case (a PQ-authenticated transaction) free of extra machinery.
