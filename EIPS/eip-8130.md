...
The protocol validates signatures by reading `actor_config` directly and delegating authentication to [Authenticators](#authenticators) — see [Validation](#validation) for the full flow. Actor enumeration is performed off-chain via `ActorAuthorized` and `ActorRevoked` event logs; for any `actorId` the most recent of these reflects the actor's current authorization. 
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Authorize an actor (upsert): an already-configured `actorId` is overwritten in place, with its `policy_commitment`/`policy_manager` slots reset before the new values are applied. Writes `actor_config` with `authenticator`, `scope`, `expiry` (`0` = no expiry), and `policyType`. Slices `policyData` into `policy_commitment`/`policy_manager`: empty for `0x00`, `manager ‖ commitment` when non-zero (see [Actor Policies](#actor-policies)); rejects mismatched `len(policyData)`, and rejects a policy-bearing actor whose `scope` is unrestricted (`0x00`) or includes CONFIG (`0x08`). Emits `ActorAuthorized`. |
