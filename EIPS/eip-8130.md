...
| `account_changes_cost` | Splits into two shares. **Validation share** (charged on every inclusion, including when phase 0 reverts): per config-change entry, the entry's auth authentication cost (same model as `sender_auth_cost`) plus one sequence-check SLOAD (2,100); chained entries are each charged in full. **Application share** (charged only when call phase 0 commits, i.e. the account changes persist): the storage write costs as previously specified — per initial or authorized actor slot 22,100 (plus 22,100 each for `policy_manager`/`policy_commitment` when `POLICY` is set), per delegation entry 4,600, and for a create entry `bytecode_cost` (32,000 + 200 per deployed byte). Per skipped (already-applied) entry: 2,100. |
| `auto_delegation_cost` | Delegation indicator deposit: 4,600 gas (200 × 23 bytes for the `0xef0100 \|\| address` indicator) when a code-less `sender` is auto-delegated to `DEFAULT_ACCOUNT_ADDRESS` (Block Execution step 5). 0 otherwise. |

**Application charging.** The application share plus `bytecode_cost` is statically derivable from the parsed entries (slot counts follow from change types and `POLICY` bits; deposit cost from the create entry's code length), so it is a known amount rather than an estimate. It is charged when `account_changes` are applied, immediately before call phase 0. Because it is statically known and covered by the intrinsic-gas check, application cannot run out of gas mid-write. If phase 0 reverts, the account changes are rolled back and this share is refunded to the payer. Future change types MUST keep the application share statically derivable from the entry encoding.

**Charging on a phase-0 revert.** When call phase 0 reverts, the transaction is still charged: the base and payload costs, `nonce_key_cost`, `sender_auth_cost`, the `account_changes` validation share, `auto_delegation_cost` if performed, the gas phase 0 consumed to the revert, and `payer_auth_cost` (outside `gas_limit`, as specified). The `account_changes` application share and all other unused gas are refunded to the payer. A transaction cannot revert its way out of paying for authorization it consumed.
...
**Effective actor state.** All authorization in this specification — actor resolution, scope, expiry, nonce scope, and the policy-gate target — is evaluated against the transaction's effective actor state: persisted Account Configuration storage statically composed with the transaction's own `account_changes`, applied in order. Composition requires validating each config-change entry's auth and sequence (see [Intrinsic Gas](#intrinsic-gas)) but executes no EVM code; authorization derives from the signature chain, not from persistence. The composed changes are applied to storage immediately before call phase 0 executes, so EVM-visible state during phase 0 and later phases (e.g. ERC-1271 via account code) reflects the applied changes — unless phase 0 reverts, which rolls them back.
...
**Phase 0 and account changes.** `account_changes` are applied immediately before call phase 0 (`calls[0]`) and are atomic with it: if phase 0 reverts, all account changes are reverted along with it, the remaining phases are skipped, and the storage-application cost is not charged (see [Intrinsic Gas](#intrinsic-gas)). To decouple account changes from the calls — applying them regardless of call outcomes — include an empty phase 0 (`[[], ...]`), which cannot revert; empty `calls` behaves identically. Because account changes apply first, an account created by this transaction's create entry already has its code and actors while phase 0 runs.

The policy gate (when the authenticating actor has `POLICY` set) applies to every call in every phase, including phase 0, with the allowed target resolved once from effective actor state (see [Validation](#validation)).
...
3. If `nonce_key != NONCE_KEY_MAX`, increment the nonce for `(sender, nonce_key)`; otherwise apply nonce-free replay processing. (Unchanged.)
4. Set transaction context on the Transaction Context precompile (sender, payer, and actorId).
...
- `status` (uint8): `0x01` = call phase 0 succeeded and durable transaction state persisted — `account_changes` were applied (if any). Later phase reverts do **not** clear `status`; consult `phaseStatuses` for per-phase outcomes. `0x00` = call phase 0 reverted — `account_changes` were rolled back and their application share was not charged. Empty `calls` or an empty phase 0 (`[[], ...]`) succeed vacuously and yield `status = 0x01`. Auto-delegation, nonce consumption, and gas payment persist regardless of `status`. `gasUsed` includes `payer_auth_cost` (see [Intrinsic Gas](#intrinsic-gas)).
- `phaseStatuses` (uint8[]): Per-phase status array. Each entry is `0x01` (success) or `0x00` (reverted). Phases after a revert are not executed and reported as `0x00`. Empty if `calls` was empty. `phaseStatuses[0]` is the phase-0 result and equals `status`.
...
### Why Are Account Changes Atomic With Phase 0?

A sponsor's gas commitment is unconditional while its compensation (canonically a phase-0 token transfer) is an execution outcome. If account changes committed independently of the calls, a sender could invalidate the payment after co-signature yet keep a payer-funded account deployment. Making `account_changes` atomic with call phase 0 — rolling them back together on a phase-0 revert, and not charging the storage-application cost in that case — bounds a sponsor's worst-case loss to a small constant that is a pure function of co-signed fields and closes the paid-for-create leak. An empty phase 0 opts out, applying account changes unconditionally for transactions with nothing to gate.

### Why Charge Validation Unconditionally?

Config-change entry signatures authorize the transaction under effective actor state whether or not the changes persist; charging them only on application would let a transaction consume arbitrary authenticator work and revert out of paying for it.
...
**Surviving signed changes**: A config-change entry carried by a transaction whose phase 0 reverted did not consume its sequence and remains a valid signed artifact; per the anyone-can-relay property it may be applied by third parties via `applySignedActorChanges` independently of any retry. Wallets MUST NOT assume a reverted transaction's payload is dead; the skip rule handles a retry that finds the change already applied.

**Visibility of account changes**: `account_changes` apply before call phase 0, so a revoked or rotated actor is already inactive during phase 0 and later phases. If phase 0 reverts, the changes roll back with it and no revocation persists; exposure is one transaction wide and mirrors the existing behavior of `sender_auth` itself.

**Gated keys and payment**: The policy gate covers phase 0 with no exemptions; a `POLICY` actor's phase-0 calls may target only its `policy_manager`. Note that `POLICY | SPONSOR_PAYER` composes as specified — sponsor authority is not gated — so such a key may co-sign as payer for third parties while unable to originate token payments for its own account.
