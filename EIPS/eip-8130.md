...
- The account's code begins with the [EIP-7702](./eip-7702.md) delegation indicator (`0xef0100`). A delegated EOA's authorization is its (mutable) delegate, and such accounts already have the implicit-EOA and [Delegation Entry](#delegation-entry) paths available.
...
AA_TX_TYPE || rlp([
  ...
  calls,              // [[{to, data}, ...], ...] | empty
  payer,              // empty = self-pay, 0x00 = open (any payer), payer_address = specific payer
  sender_auth,        // raw ECDSA r||s||v (EOA, sender empty) | authenticator || data (configured actor)
  payer_auth          // empty = self-pay / open-coinbase | r||s||v (65) = open recovered payer | authenticator || data = sponsored
])
...
| `payer` | Gas payer identity. **Empty**: Sender pays. **`0x00`** (single byte, `OPEN_PAYER_SENTINEL`): Open — the sender authorizes any payer; the actual payer is resolved from `payer_auth`. **20-byte address**: This specific payer required. See [Payer Modes](#payer-modes) |
| `sender_auth` | See [Signature Format](#signature-format) |
| `payer_auth` | Payer authorization. **Empty**: self-pay, or — when `payer` is `0x00` — the open builder path (payer is `block.coinbase`). **65-byte `r \|\| s \|\| v`** (only when `payer` is `0x00`): open recovered payer — the secp256k1 signer of the payer hash is the payer. **`authenticator \|\| data`**: sponsored — same format as `sender_auth`. See [Payer Modes](#payer-modes) |
...
**`payer_auth_cost`**: 0 for self-pay (`payer` empty) and for the open builder path (`payer` is `0x00` with empty `payer_auth`, where `block.coinbase` is the payer and no payer signature is verified). For the open recovered payer (`payer` is `0x00` with a 65-byte `r || s || v`), the native secp256k1 recovery cost applies (the `ECRECOVER_AUTHENTICATOR` model). Otherwise, the same `sender_auth_cost` model applies to the payer's authenticator.
...
| `0x00` (1 byte) | Open | Sender authorizes **any** payer; the actual payer is resolved from `payer_auth`. See [Open Payer Mode](#open-payer-mode) |
| `payer_address` (20 bytes) | Sponsored | Sender binds tx to a specific sponsor |

**`payer_auth`** — uses the same `authenticator || data` format as `sender_auth`, except in the open recovered case where it is a bare 65-byte secp256k1 signature:
...
| `0x00` | empty | `block.coinbase` | Open / builder — no `payer_auth` to verify; the builder of the including block (`block.coinbase`) is the payer, authorized by its act of inclusion. Builder-direct path (not gossiped as a normally-sponsored tx) |
| `0x00` | `r \|\| s \|\| v` (65 bytes) | `ecrecover(payer_hash, sig)` | Open / recovered — the secp256k1 signer of the [payer signature hash](#signature-payload) is the payer and MUST have sufficient balance. Recovered natively; no `actor_config` lookup or PAYER scope — the signature is the authorization |
...
**Policy gate**: When the transaction's authenticating actor has `policyType != 0x00`, each call is gated before dispatch. The protocol resolves the actor's allowed target — `policy_manager(sender, actorId)` — and if `call.to` is not that address, the call is **not dispatched** and fails deterministically with the protocol revert `ActorPolicyViolation(bytes32 actorId, address target)`:
...
| `getTransactionPayer()` | `address` — gas payer (`sender` for self-pay; `block.coinbase` or recovered signer in [open mode](#open-payer-mode); payer for sponsored) | Execution only |
...
6. Resolve payer from `payer` and `payer_auth`:
   ...
   - `payer` = `0x00` (open) and `payer_auth` empty: open builder path. The payer is `block.coinbase` of the including block, authorized by the builder's act of inclusion; there is no `payer_auth` to verify. Because the payer is not determined until a block is built, this variant is **not** admitted to the public mempool for gossip — it is offered directly to builders, which self-protect by validating reimbursement off-chain before inclusion (see [Open Payer Mode](#open-payer-mode)).
   - `payer` = `0x00` (open) and `payer_auth` = 65-byte `r || s || v`: open recovered payer. Recover the payer address from the [payer signature hash](#signature-payload) via native secp256k1. The recovered address is the payer and MUST have sufficient balance; no `actor_config` lookup or PAYER scope is required, as the signature itself authorizes payment. This variant has an identified payer and is subject to normal per-payer mempool accounting.
...
2. ETH gas deduction from payer (`sender` for self-pay; `block.coinbase` for the open builder path; the recovered signer for the open recovered payer). Transaction is invalid if payer has insufficient balance.
...
- `payer` (address): Gas payer address (`sender` for self-pay; resolved `block.coinbase` or recovered signer in [open mode](#open-payer-mode); specified payer for sponsored).
...
| `OPEN_PAYER_SENTINEL` | `0x00` (single byte) | `payer` value selecting [open mode](#open-payer-mode); distinct from empty (self-pay) and from any 20-byte address |
...
Locked accounts have a frozen actor set, so the primary state that can invalidate a validated transaction is nonce consumption. This can enable nodes to cache actor state and apply higher mempool rate limits (see [Mempool Acceptance](#mempool-acceptance)).
...
The committed payer mode binds a transaction to one specific payer address before the sender signs. That works when the payer is known and reachable (an app backend, or an L2 sequencer at a fixed address), but it is a poor fit for a chain-agnostic, L1-robust token-payment platform: the sender must know the payer's address at signing time, and if that payer is unavailable the transaction must be re-signed for a different one. Open mode (`payer = OPEN_PAYER_SENTINEL`, the single byte `0x00`) lets the sender authorize *any* payer with one signature, deferring the choice of payer to inclusion time. It resolves to one of two payers:

- **Builder (`payer_auth` empty)** — the payer is `block.coinbase`. `block.coinbase` is fixed in the block header before execution and constant throughout the block, so it is well-defined for `getTransactionPayer()`. There is no `payer_auth` to verify: the entity being paid is the same entity that chose to include the transaction, so its act of inclusion (committed by the block's consensus signature) *is* the authorization. This makes the protocol's native fee-abstraction story keyless for the builder and removes a payer-service round trip — on an L2 the wallet builds, signs, and submits straight to the sequencer.
- **Recovered (`payer_auth` is a 65-byte `r || s || v`)** — the payer is the secp256k1 signer of the payer hash, recovered natively. Any account willing to pay signs the payer hash (which binds to the resolved sender, calls, gas, and expiry) and must hold the balance. This supports a permissionless payer market: because the sender committed only to "open," several payers can attach their own `payer_auth` to the *same* sender-signed transaction, and the nonce guarantees only one lands — fallback needs no re-sign.

**Reimbursement.** Because the payer address is not known at signing time, a phase-0 reimbursement cannot name a concrete recipient. Tokens can expose a `transferToPayer(amount)` method that transfers to `getTransactionPayer()` from the [Transaction Context](#transaction-context) precompile, resolving the recipient at execution. A payer self-protects off-chain: before signing (recovered) or including (builder), it checks that the phase-0 reimbursement covers `gas_limit × max_fee_per_gas` at its accepted rate and that the sender holds the balance, and otherwise drops the transaction with no on-chain effect. The reimbursement is an ordinary committed phase-0 call, so it persists even if later phases revert — exactly the property gas payment needs.

**Why this is not a permissionless paymaster.** The mode that earlier 8130 drafts removed ran payer-supplied validation code in-protocol, which forced simulation and created mempool DoS. Open mode runs no payer code during validation: authorization is either a plain signature (recovered) or an inclusion decision (builder), both made off-chain. The DoS surface that motivated dropping permissionless paymasters is simply absent. The one operational caveat is mempool attribution — the builder variant has no identified payer until a block is built, so it is a builder-direct path rather than a gossiped transaction, while the recovered variant carries an identified payer and uses normal per-payer accounting.
...
**Open Payer Risk**: In [open mode](#open-payer-mode) the sender authorizes any payer, so payer safety rests entirely on the payer's own off-chain decision. The recovered variant relies on the same resolved-sender binding above — the payer signs the payer hash for a specific resolved sender, so its signature cannot be replayed across senders — and the payer pays only after verifying reimbursement, so it never signs a transaction that fails to compensate it. The builder variant carries no third-party payer: `block.coinbase` is both the payer and the includer, so it only ever pays for transactions it chose to include. Senders are not exposed by open mode: they cap their cost via the signed phase-0 reimbursement (e.g. `transferToPayer(amount)`) regardless of which payer fills `payer_auth`.
