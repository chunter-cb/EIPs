...
AA_TX_TYPE || rlp([
  ...
  payer,              // empty = self-pay | 0x00 = open (payer recovered) | payer_address = specific payer
...
| `payer` | Gas payer identity. **Empty**: sender pays (self-pay). **`0x00`** (single sentinel byte): open — the sender commits to being sponsored without naming a payer; the payer is recovered from `payer_auth` (k1 only). **20-byte address**: this specific payer required. See [Payer Modes](#payer-modes) |
| `sender_auth` | See [Signature Format](#signature-format) |
| `payer_auth` | Payer authorization. **Empty**: self-pay. **Non-empty**: `authenticator || data` — same format as `sender_auth`. In open mode (`payer == 0x00`) the authenticator MUST be `K1_AUTHENTICATOR` and `data` is raw ECDSA `(r || s || v)`. See [Payer Modes](#payer-modes) |
...
| `0x00` (sentinel byte) | Open | Sender accepts sponsorship without committing to a payer; the payer is recovered from `payer_auth` |
...
| `0x00` | `K1_AUTHENTICATOR (20) \|\| r,s,v (65)` | recovered from `payer_auth` | Open — k1 only. The payer address is `bytes32(bytes20(recovered_address))`; reads that payer's `actor_config` and requires PAYER scope |
...
Open mode is restricted to `K1_AUTHENTICATOR` because k1 derives the payer address from the signature alone (ecrecover), so no committed `payer` address is needed to identify the account — mirroring the EOA sender path (`sender` empty). Other authenticators require an explicit `payer` address to look up `actor_config`, so they are only valid in sponsored mode.
...
| `getTransactionPayer()` | `address` — gas payer (`sender` for self-pay, recovered/specified payer otherwise) | Execution only |
...
6. Resolve payer from `payer` and `payer_auth`:
   ...
   - `payer` = `0x00` (open): `payer_auth` MUST use `K1_AUTHENTICATOR`; reject otherwise. Ecrecover the payer address from `payer_auth` (`data` as `r || s || v`); the payer is the recovered address. Validate against that payer's `actor_config` and require PAYER scope on the resolved actor.
...
- `payer` (address): Gas payer address (`sender` for self-pay, recovered payer for open, specified payer for sponsored).
