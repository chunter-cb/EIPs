...
An authenticator MAY be designated **aggregatable**, meaning the protocol defers its cryptographic signature check to a single block-level recursive STARK rather than performing it inline. This makes expensive signature schemes practical on the 8130 path: a post-quantum leanSPHINCS verification that costs ~150,000 gas inline is instead declared as a lightweight **dependency** and proven, together with every other aggregatable signature in the block, by one proof. This section is the complete specification of aggregation.

Aggregation is **only** an optimization of signature verification. It does not introduce a new authentication model — it reuses the existing [Authenticators](#authenticators) and [Validation](#validation) flow, deferring a single step. Concretely it adds: a class of sentinel authenticator, a per-transaction `dependencies` field, one block-level validity rule (the recursive STARK), and one [Transaction Context](#transaction-context) query (`isDependencyProven`). Everything else — authorization, scope, nonce, policy, ordering — is unchanged.

A block dependency is a `(scheme, data_hash, verification_key)` triple, and comes from one of two sources:

- **Implicit — transaction authentication.** An aggregatable authenticator used in `sender_auth`, `payer_auth`, or a config-change `auth`. The triple is derived from the auth blob; nothing extra is carried (see [Aggregatable Authenticators](#aggregatable-authenticators)).
- **Explicit — in-EVM signature checks.** Triples the transaction lists up front in its `dependencies` field, consumed during execution by contract code (e.g. [ERC-1271](./eip-1271.md)); see [ERC-1271 and In-EVM Signatures](#erc-1271-and-in-evm-signatures).
...
The explicit `dependencies` transaction field is a `96*k` concatenation of these triples. Implicit dependencies (from aggregatable authenticators) are the same triple, materialized by the protocol rather than carried in the field: `data_hash` is the relevant signature hash and `verification_key` is the auth blob's key. The complete dependency set of a transaction is its implicit triples followed by its explicit ones; the block's set is the concatenation across all transactions in block order.
...
The block is valid only if `block_deps_hash` equals the hash of the dependencies collected from all AA transactions (in block order) and `stark_proof` verifies against `block_deps_hash` and the protocol-level `AGGREGATED_VK`. The proof uses the "verify, merge, and discard" recursive construction of [EIP-8288](./eip-8288.md). This proposal adapts that construction: EIP-8288 declares dependencies through [EIP-8141](./eip-8141.md) frames, whereas here the authenticator already names the actor (implicit deps) or the transaction lists them (explicit deps), so no frame layer is required.
...
2. **Dependency set tracks the included set.** `block_deps_hash` MUST contain the dependencies of exactly the included transactions — no more, no less. When a transaction is dropped, its implicit and explicit dependencies MUST be removed from the set (the recursive construction's *discard* mechanism), so the proof does not attest a signature for a transaction that is not in the block.
...
Signatures verified *inside* the EVM — most importantly [ERC-1271](./eip-1271.md) `verifySignature()`, but also any contract that checks a signature — are handled in one of two ways.

**Inline (baseline, always available).** The full signature is passed to the verifier and checked on the spot. This works everywhere, including off-chain `eth_call`, and requires none of the machinery in this section; a post-quantum signature simply pays its full inline cost (~150,000 gas for leanSPHINCS). Aggregation is never mandatory for in-EVM checks.

**Aggregated (opt-in, via declared dependencies).** To get the aggregation discount for an in-EVM check, the transaction declares the corresponding triple in its `dependencies` field. The verifier then, instead of running the algorithm, asks the [Transaction Context](#transaction-context) precompile `isDependencyProven(scheme, data_hash, verification_key)`. Here `data_hash` is the **application** hash being checked (an order or permit hash), not the transaction hash, and the signer's real signature travels as a witness in the mempool wrapper. For an aggregated ERC-1271 check the account's `verifySignature()` resolves `actorId = keccak256(verification_key)`, enforces SIGNATURE scope on `actor_config` **inline** (unchanged), and consults `isDependencyProven` for the cryptography.

The precompile only reports whether a triple was **declared** by the transaction — it does not re-verify the STARK. Soundness comes from [Block Validity](#block-validity): every declared dependency is folded into `block_deps_hash`, and the block is invalid unless the recursive STARK proves it. Therefore, **on any valid block, "declared" implies "proven,"** and a `true` result is safe. Two consequences follow: a check whose triple was not declared MUST fall back to inline verification (never silently pass), and aggregated in-EVM verification is meaningful only inside a block — off-chain `eth_call` has no block proof and uses the inline path.

#### Mempool

A node cannot verify a dependency from the transaction alone: implicit dependencies carry only the `verification_key` in the auth blob, and explicit `dependencies` carry only the triple. The signatures (witnesses) travel separately. Such transactions are gossiped inside an [EIP-8288](./eip-8288.md) **wrapper object** that bundles transactions with either their individual proofs or a recursive STARK, and that supports recursive re-aggregation between peers. A node MUST hold a valid proof for every dependency — implicit and explicit — before accepting the transaction, and SHOULD enforce the per-transaction limits `MAX_SIGS_PER_TX` and `MAX_STARKS_PER_TX`. All non-cryptographic mempool checks ([Mempool Acceptance](#mempool-acceptance)) are unchanged.

#### Gas

An aggregatable authenticator is priced as an enshrined authenticator (see [Authenticators](#authenticators)) with a fixed, deferred verification cost in place of inline execution. The per-transaction `sender_auth_cost` / `payer_auth_cost` for an implicit dependency is:

```
aggregatable_auth_cost = AGGREGATED_VERIFICATION_GAS[scheme] + 2,100   // fixed scheme cost + 1 cold actor_config SLOAD
```

Explicit dependencies (the `dependencies` field) pay for their bytes through the standard `tx_payload_cost` (see [Intrinsic Gas](#intrinsic-gas)) and, like implicit ones, `AGGREGATED_VERIFICATION_GAS[scheme]` per declared triple.
...
AA_TX_TYPE || rlp([
  ...
  dependencies,       // declared aggregation dependencies for in-EVM checks | empty — see Signature Aggregation
...
| `dependencies` | **Empty**: No declared aggregation dependencies. **Non-empty**: `96*k`-byte concatenation of `(scheme, data_hash, verification_key)` triples proven by the block's recursive STARK and readable during execution via the [Transaction Context](#transaction-context) precompile. Enables aggregated in-EVM signature checks (e.g. [ERC-1271](./eip-1271.md)). See [Signature Aggregation](#signature-aggregation) |
...
keccak256(AA_TX_TYPE || rlp([
  ...
  account_changes, calls, metadata, dependencies,
  payer
]))
```
...
| `isDependencyProven(scheme, dataHash, verificationKey)` | `bool` — whether the triple is among the transaction's declared `dependencies` (and therefore proven by the block's recursive STARK; see [Signature Aggregation](#signature-aggregation)) | Execution only |
...
**Why the authenticator seam rather than frames.** EIP-8288 layers on [EIP-8141](./eip-8141.md) and needs a dedicated frame mode to *declare* which signatures to prove. 8130 already routes every authentication through an authenticator that names the actor, so tx-level authentication needs no new declaration surface — the auth blob is the declaration. Only in-EVM checks (which the protocol cannot see ahead of time) need the explicit `dependencies` field. This keeps the common case (a PQ-authenticated transaction) free of extra machinery.
...
interface ITransactionContext {
    ...
    // Aggregation: true if (scheme, dataHash, verificationKey) is among the
    // transaction's declared `dependencies`. On a valid block a declared
    // dependency is proven by the block's recursive STARK, so this doubles as
    // "was this signature proven at the block level?" See Signature Aggregation.
    function isDependencyProven(uint8 scheme, bytes32 dataHash, bytes32 verificationKey)
