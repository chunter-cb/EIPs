...
The canonical authenticator set and corresponding contract addresses are maintained in a companion ERC (number TBD) and deployed at deterministic CREATE2 addresses across chains. The canonical set is expected to grow as new algorithms are adopted through the companion ERC process. Expensive schemes — notably post-quantum signatures — can be made practical by deferring their verification to a block-level proof; see [Signature Aggregation](#signature-aggregation).
...
An authenticator MAY be designated **aggregatable**, meaning the protocol defers its cryptographic signature check to a single block-level recursive STARK rather than performing it inline. This makes expensive signature schemes practical on the 8130 path: a post-quantum leanSPHINCS verification that costs ~150,000 gas inline is instead recorded as a lightweight **dependency** and proven, together with every other aggregatable signature in the block, by one proof. This section is the complete specification of aggregation.

Aggregation is **only** an optimization of signature verification, and it requires **no change to the [AA transaction type](#aa-transaction-type)**. Concretely, aggregation adds: a class of sentinel authenticator, a system precompile at `AGGREGATION_ADDRESS`, and one block-level validity rule (the recursive STARK). Everything else — the wire format, authorization, scope, nonce, policy, ordering — is unchanged.

A block dependency is a `(scheme, data_hash, verification_key)` triple. It comes from one of two sources, **both expressed inside existing transaction fields** so the transaction format is untouched:

- **Transaction authentication.** An aggregatable authenticator used in `sender_auth`, `payer_auth`, or a config-change `auth`. The protocol derives the triple from the auth blob; nothing extra is carried (see [Aggregatable Authenticators](#aggregatable-authenticators)).
- **In-EVM signature checks.** A top-level entry in the existing `calls` array that calls `AGGREGATION_ADDRESS` with the triples as calldata. Because `calls` is part of the signed transaction, these declarations are visible statically — before execution — so they are known up front just like auth-derived ones. This is how [ERC-1271](./eip-1271.md) and other in-EVM checks are aggregated (see [ERC-1271 and In-EVM Signatures](#erc-1271-and-in-evm-signatures)).

Both sources feed the same block-level dependency set and the same proof.

#### Aggregatable Authenticators

Aggregatable authenticators are protocol-reserved sentinels, like `K1_AUTHENTICATOR`. Each names a verification scheme handled by [Lean Ethereum](https://github.com/leanEthereum) tooling:

| Name | Address | Scheme byte | `data` (in-block) | `actorId` |
|------|---------|-------------|-------------------|-----------|
| leanSPHINCS | `LEANSPHINCS_AUTHENTICATOR` (`address(2)`) | `0x10` | `verification_key` (32 bytes, the pubkey) | `keccak256(verification_key)` |
| leanSTARK | `LEANSTARK_AUTHENTICATOR` (`address(3)`) | `0x11` | `verification_key` (32 bytes, the STARK vk) | `keccak256(verification_key)` |

Like `K1_AUTHENTICATOR`, these are protocol sentinels rather than deployed contracts: the protocol recognizes the address and handles it natively instead of issuing a STATICCALL. They are part of the [Canonical Authenticator Set](#canonical-authenticator-set) and added through the same companion ERC process.

#### Dependency Encoding

A dependency is a 96-byte triple, encoded identically to [EIP-8288](./eip-8288.md) so the two share proving tooling:

```
dependency (96 bytes) =
    scheme            // 31 zero bytes || 1 scheme byte (0x10 leanSPHINCS, 0x11 leanSTARK)
 || data_hash         // 32 bytes: message hash (leanSPHINCS) or public-inputs hash (leanSTARK)
 || verification_key  // 32 bytes: pubkey (leanSPHINCS) or STARK vk (leanSTARK)
```

No new transaction field carries these triples. For **transaction authentication**, the protocol materializes the triple from the aggregatable authenticator's auth blob: `data_hash` is the relevant signature hash (see [What Aggregation Defers](#what-aggregation-defers)) and `verification_key` is the auth blob's key. For **in-EVM checks**, the triples are the calldata of a top-level `calls` entry addressed to `AGGREGATION_ADDRESS`, and `data_hash` is the application hash being verified (an order or permit hash), not a transaction hash. A transaction's dependency set is the concatenation of its auth-derived triples and its `AGGREGATION_ADDRESS`-declared triples; the block's set is the concatenation across all transactions in block order.

#### What Aggregation Defers

Aggregation defers **only** the cryptographic signature check. Every other part of validation — resolving `actorId`, the `actor_config` SLOAD and authenticator binding, scope, expiry, nonce, and policy — is evaluated inline and in sequence, exactly as for any other authenticator (see [Validation](#validation)). A signature's cryptographic validity is independent of execution order, so it is safe to prove once per block; authorization is state-dependent, so it stays inline.

When the protocol encounters an aggregatable sentinel as the effective authenticator in `sender_auth`, `payer_auth`, or a config-change `auth`, it replaces step 2 of [Validation](#validation) (the authenticate call) with:

1. Parse `data` as the 32-byte `verification_key`.
2. Resolve `actorId = keccak256(verification_key)`. Steps 3–4 of [Validation](#validation) (authorize, check scope/expiry against `actor_config`) then proceed unchanged.
3. Record the dependency triple `(scheme, sender_signature_hash, verification_key)` for the block, where `sender_signature_hash` is the value defined in [Signature Payload](#signature-payload) (for `payer_auth`, the payer signature hash; for a config-change `auth`, the [Config Change Signature Payload](#config-change-signature-payload)). **No signature is verified inline** — its validity is established by the block proof below.

The signature bytes are never carried in the block; they travel in the mempool wrapper and are absorbed into the block proof.

#### Validity Timing

Because the proof is built once over the block's whole dependency set, the cryptographic check is effectively evaluated at the top of the block rather than at each transaction's position. This is sound precisely because aggregation is limited to signature verification: a valid signature is valid regardless of where its transaction sits.

Authorization is unaffected. If an earlier transaction's in-EVM config change (e.g. `applySignedActorChanges()`) revokes or rescopes the signing actor, a later transaction signed by that actor still fails the inline `actor_config` check at its own position — even though its signature was aggregated and proven. Aggregation never lets a transaction bypass the in-order authorization state; the only thing proven block-wide is "this `verification_key` signed this `data_hash`."

#### Block Validity

A block carrying aggregated dependencies MUST include a recursive STARK in its header proving all of them:

```
recursive_stark = [stark_proof, block_deps_hash]

block_deps_hash = keccak256(concat(
    bytes32(scheme) || data_hash || verification_key   // for each dependency, in block order
))
```

The block is valid only if `block_deps_hash` equals the hash of the dependencies collected from all AA transactions (in block order) and `stark_proof` verifies against `block_deps_hash` and the protocol-level `AGGREGATED_VK`. The proof uses the "verify, merge, and discard" recursive construction of [EIP-8288](./eip-8288.md). This proposal adapts that construction: EIP-8288 declares dependencies through [EIP-8141](./eip-8141.md) frames, whereas here the declaration sites are the existing auth blob and `AGGREGATION_ADDRESS` calls, so no frame layer — nor any transaction-format change — is required.

#### Builder Requirements

Because authorization is checked inline and in order while the proof is built over the block set as a whole, the builder MUST keep the two consistent:

1. **Order-aware inclusion.** A transaction whose signing actor is revoked or rescoped by an *earlier* transaction in the same block (via an in-EVM config change) fails its inline `actor_config` check and is not a valid inclusion. The builder MUST simulate in order and exclude such a transaction, exactly as it would for any transaction that becomes invalid mid-block.
2. **Dependency set tracks the included set.** `block_deps_hash` MUST contain the dependencies of exactly the included transactions — no more, no less. The builder collects each transaction's dependencies statically: auth-derived triples from its aggregatable auth blobs, and in-EVM triples from the calldata of its top-level `AGGREGATION_ADDRESS` calls (no execution required to discover them). When a transaction is dropped, its dependencies MUST be removed from the set (the recursive construction's *discard* mechanism), so the proof does not attest a signature for a transaction that is not in the block.

These are the only aggregation-specific obligations beyond ordinary block building; a transaction's signature being cryptographically valid never forces or permits its inclusion.

#### ERC-1271 and In-EVM Signatures

Signatures verified *inside* the EVM — most importantly [ERC-1271](./eip-1271.md) `verifySignature()`, but also any contract that checks a signature — can be aggregated too, via the `AGGREGATION_ADDRESS` precompile, with an always-available inline fallback.

**Inline (baseline).** The full signature is passed to the verifier and checked on the spot. This works everywhere, including off-chain `eth_call`, and a post-quantum signature pays its full inline cost (~150,000 gas for leanSPHINCS). Aggregation is never mandatory for in-EVM checks.

**Aggregated (via `AGGREGATION_ADDRESS`).** The wallet declares the dependency by including a **top-level call** to `AGGREGATION_ADDRESS` in `calls` — conventionally the first call — whose calldata is the `96*k` concatenation of `(scheme, data_hash, verification_key)` triples the transaction relies on. Then:

1. The builder and mempool read those triples **statically** from the signed `calls` array and add them to the block dependency set; the recursive STARK proves them. The actual signatures (witnesses) travel in the mempool wrapper. This is what avoids dynamic mid-execution discovery: a dependency is declared as calldata to a known precompile, visible before any code runs.
2. During execution, a verifier calls `AGGREGATION_ADDRESS.isProven(scheme, data_hash, verification_key)` instead of running the algorithm. For an aggregated ERC-1271 check, the account's `verifySignature()` resolves `actorId = keccak256(verification_key)`, enforces SIGNATURE scope on `actor_config` **inline** (unchanged), and consults `isProven` for the cryptography.

`isProven` reports whether a triple is among the transaction's declared dependencies; it does not re-verify the STARK. Soundness comes from [Block Validity](#block-validity): every declared triple is folded into `block_deps_hash`, and the block is invalid unless the recursive STARK proves it. Therefore, **on any valid block, "declared" implies "proven,"** and a `true` result is safe. Two consequences follow: only **top-level** `calls` entries to `AGGREGATION_ADDRESS` are declarations (a nested call cannot be seen statically, so it MUST NOT create a dependency), and a check whose triple was not declared MUST fall back to inline verification — including off-chain `eth_call`, which has no block proof to consult.

#### Mempool

A node cannot verify a dependency from the transaction alone: an auth-derived triple carries only the `verification_key` in the auth blob, and an `AGGREGATION_ADDRESS`-declared triple carries only the triple in the call's calldata. The signatures (witnesses) travel separately. Such transactions are gossiped inside an [EIP-8288](./eip-8288.md) **wrapper object** that bundles transactions with either their individual proofs or a recursive STARK, and that supports recursive re-aggregation between peers. A node MUST hold a valid proof for every dependency — auth-derived and `AGGREGATION_ADDRESS`-declared — before accepting the transaction, and SHOULD enforce the per-transaction limits `MAX_SIGS_PER_TX` and `MAX_STARKS_PER_TX`. All non-cryptographic mempool checks ([Mempool Acceptance](#mempool-acceptance)) are unchanged.

#### Gas

An aggregatable authenticator is priced as an enshrined authenticator (see [Authenticators](#authenticators)) with a fixed, deferred verification cost in place of inline execution. The per-transaction `sender_auth_cost` / `payer_auth_cost` is:

```
aggregatable_auth_cost = AGGREGATED_VERIFICATION_GAS[scheme] + 2,100   // fixed scheme cost + 1 cold actor_config SLOAD
```

The block additionally charges, like intrinsic calldata gas (and so unaffected by execution reverting):

```
recursive_stark_gas = LEANSTARK_VERIFICATION_GAS * total_block_dependencies
```

where `total_block_dependencies` counts every dependency in the block. The fixed per-scheme values above are a **recommended schedule**, not protocol constants (see [Intrinsic Gas](#intrinsic-gas)): they should track the *amortized* prover cost of one more dependency, which is low for STARK-friendly schemes (hash-based leanSPHINCS, leanSTARK) and high for elliptic-curve schemes (see [Why Signature Aggregation](#why-signature-aggregation)). The on-chain verifier cost is a single recursive proof per block regardless of dependency count.

| Constant | Value | Comment |
|----------|-------|---------|
| `LEANSPHINCS_AUTHENTICATOR` | `address(2)` | Native sentinel for aggregated leanSPHINCS (post-quantum) signatures |
| `LEANSTARK_AUTHENTICATOR` | `address(3)` | Native sentinel for aggregated leanSTARK proofs |
| `AGGREGATION_ADDRESS` | `0x813000000000000000000000000000000000aa03` | Aggregation precompile: in-EVM dependency declaration and `isProven` query |
| `LEANSPHINCS_VERIFICATION_GAS` | 3,000 | Deferred per-signature cost for leanSPHINCS |
| `LEANSTARK_VERIFICATION_GAS` | 30,000 | Deferred per-proof cost for leanSTARK |
| `MAX_SIGS_PER_TX` | 16 | Max aggregated leanSPHINCS dependencies per transaction |
| `MAX_STARKS_PER_TX` | 1 | Max aggregated leanSTARK dependencies per transaction |
| `AGGREGATED_VK` | TBD | Protocol-level verification key for the recursive STARK |
...
| **Aggregation** | Precompile at `AGGREGATION_ADDRESS` + block-level recursive STARK (see [Signature Aggregation](#signature-aggregation)) | No code at address; in-EVM checks use inline verification only |
...
| `AGGREGATION_ADDRESS` | `0x813000000000000000000000000000000000aa03` | Aggregation precompile address (see [Signature Aggregation](#signature-aggregation)) |
...
Post-quantum signatures are large and expensive to verify (~150,000 gas and several kilobytes for a hash-based signature), which would make a post-quantum entry in the canonical set impractical on the 8130 path if verified inline per transaction. [Signature Aggregation](#signature-aggregation) makes it practical by declaring each such signature as a lightweight dependency and proving all of them in one block-level recursive STARK, reusing the [EIP-8288](./eip-8288.md) construction and Lean Ethereum tooling.

**Why no transaction-format change.** EIP-8288 layers on [EIP-8141](./eip-8141.md) and needs a dedicated frame mode to *declare* which signatures to prove. 8130 already has two signed surfaces that serve as declaration sites, so it needs neither frames nor a new transaction field: the authenticator that authorizes the transaction (for transaction authentication), and the `calls` array (for in-EVM checks, via a top-level call to `AGGREGATION_ADDRESS`). Both are part of the signed transaction and visible to the builder and mempool *before* execution, which is the property aggregation actually requires — the set of signatures to prove must be known up front so the recursive STARK can be built and so the mempool can reject a transaction whose dependency lacks a proof. Routing in-EVM declarations through a top-level `calls` entry (rather than allowing a dependency to appear dynamically mid-execution) preserves that static visibility while keeping the transaction format fixed.

**Why "aggregate any signature" is bounded by STARK-friendliness.** In principle any verifier expressible as a circuit can be aggregated, so aggregation could in the limit cover *every* signature in a block — collapsing the block's entire authentication layer to one proof, with large data-availability and quantum-safety wins. In practice the economics invert the usual intuition: schemes that are cheap to verify inline (secp256k1, P-256) are *expensive* to prove in a circuit (foreign-field and elliptic-curve arithmetic), while hash-based schemes (leanSPHINCS) and STARKs are cheap to prove. Aggregation is therefore clearly worthwhile for post-quantum and STARK schemes today, and only marginally (or negatively) so for legacy elliptic-curve signatures — which is why the recommended gas schedule tracks amortized prover cost rather than inline verifier cost. The whole-block endgame becomes natural once a STARK-friendly signature is the default authenticator.

**Why authorization stays inline.** Aggregation defers only the cryptographic check, which is order-independent; authorization (actor state, scope, expiry, nonce) is state-dependent and stays inline and in order. This split is what lets the proof be built once per block without weakening the guarantee that a revoked or out-of-scope actor is rejected at its position (see [Validity Timing](#validity-timing)).
...
### IAggregation (Precompile)

```solidity
interface IAggregation {
    // Declaration: a top-level `calls` entry to AGGREGATION_ADDRESS whose calldata is a
    // 96*k concatenation of (scheme, dataHash, verificationKey) triples. The builder and
    // mempool read `dependencies` statically from the signed `calls` array; each triple
    // joins the block dependency set and is proven by the block's recursive STARK.
    // The dispatched call returns true (its purpose is static declaration, not computation).
    function declare(bytes calldata dependencies) external returns (bool);

    // Query: true if (scheme, dataHash, verificationKey) is among the transaction's
    // declared dependencies. On a valid block a declared dependency is proven, so this
    // doubles as "was this signature proven at the block level?" See Signature Aggregation.
    function isProven(uint8 scheme, bytes32 dataHash, bytes32 verificationKey)
        external view returns (bool);
}
```

Only **top-level** `calls` entries to `AGGREGATION_ADDRESS` are declarations; `isProven` is callable at any depth. See [ERC-1271 and In-EVM Signatures](#erc-1271-and-in-evm-signatures).
