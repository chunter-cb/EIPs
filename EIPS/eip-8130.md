...
category: Core
created: 2025-10-14
requires: 155, 1271, 1559, 2028, 2718, 7702, 7708
---
...
Each owner occupies a single `owner_config` slot containing the verifier address, scope byte, and policy type. When `policyType == 0x01`, per-call policy commitments are held in `owner_policy(account, ownerId, callTo) → bytes32` (see [Owner Policies](#owner-policies)). Non-EOA owners are revoked by deleting the `owner_config` slot. The implicit EOA owner (`ownerId == bytes32(bytes20(account))`) is revoked by overwriting the slot with `verifier = REVOKED_VERIFIER` (`type(uint160).max`), making it distinguishable from an empty (implicitly authorized) slot.

| Field | Bytes | Description |
|-------|-------|-------------|
| `verifier` | 0–19 | Verifier contract address |
| `scope` | 20 | Permission bitmask (`0x00` = unrestricted; see [Owner Scope](#owner-scope)) |
| `policyType` | 21 | Policy selector (`0x00` = none, `0x01` = per-call mapping; see [Owner Policies](#owner-policies)) |
| reserved | 22–31 | Reserved for future use (must be zero) |

**Implicit EOA authorization**: An unregistered owner (`owner_config` slot is empty) is implicitly authorized if `ownerId == bytes32(bytes20(account))`. The empty slot's scope byte and `policyType` are both `0x00`, granting unrestricted permissions and no protocol-enforced policy by default. This allows every existing EOA to send AA transactions immediately without prior registration. When the implicit rule applies, the protocol verifies using native ecrecover rather than calling an external verifier contract. The implicit authorization is revoked by writing `REVOKED_VERIFIER` (`type(uint160).max`) to the verifier field, making the slot non-empty and blocking re-authorization. The EOA owner can also be explicitly registered with `ECRECOVER_VERIFIER` (`address(1)`) to set a custom scope or `policyType` while retaining native ecrecover verification.
...
The protocol validates signatures by reading `owner_config` directly and delegating authentication to [Verifiers](#verifiers) — see [Validation](#validation) for the full flow. Owner and policy enumeration is performed off-chain via `OwnerAuthorized`, `OwnerRevoked`, and `PolicySet` event logs. No owner or policy-entry counts are enforced on-chain — gas costs naturally bound them.

#### Owner Policies

The `policyType` byte selects an optional protocol-enforced call policy. The protocol rejects unrecognized values to keep the selector extensible.

| `policyType` | Name | Description |
|--------------|------|-------------|
| `0x00` | none | No protocol-enforced policy. |
| `0x01` | per-call mapping | Every entry in `calls` MUST have a non-zero commitment in `owner_policy(account, ownerId, callTo)`. |
| `0x02`–`0xFF` | reserved | Reserved for future modes (e.g., denylist, alternative keying); a future ERC may lift specific values. |

For `policyType == 0x01`, per-target authorization is held in a single mapping:

```
owner_policy(account, ownerId, callTo) → bytes32 commitment
```

A non-zero `commitment` authorizes calls to `callTo`; a zero value means the target is not authorized. The commitment is opaque to the protocol. Policy contracts called through `calls` retrieve it from the Account Configuration Contract together with the authenticated `ownerId` (Transaction Context precompile) and enforce their own semantics.

Enforcement applies only under SENDER scope; PAYER, SIGNATURE, and CONFIG authentication ignore `policyType`. See [Why Per-Call Policies?](#why-per-call-policies) for rationale.
...
| `nonce_key_cost` | `NONCE_KEY_MAX`: 14,000 gas (replay-protection state only resets existing slots; no new state created). Otherwise: 22,100 gas for first use of a `nonce_key` (cold SLOAD 2,100 + new-state SSTORE 20,000), 5,000 gas for existing keys (non-zero → non-zero update) |
| `bytecode_cost` | 0 if no create entry in `account_changes`. Otherwise: 32,000 (deployment base) + 200 gas per deployed byte of `code` (code deposit cost). Calldata byte costs for `code` are charged separately by `tx_payload_cost` |
| `account_changes_cost` | Per applied config change entry: auth verification cost (same model as `sender_auth_cost`) + storage write cost per mutated slot — 22,100 for a new (zero → non-zero) `owner_config` or `owner_policy` slot, 5,000 to update an existing slot, and the standard refund when a slot is cleared. Per applied delegation entry: code deposit cost (200 × 23 bytes for the delegation indicator). Per skipped config change entry (already applied): 2,100 (SLOAD to check sequence). 0 if no config change or delegation entries in `account_changes` |
...
6. **Enforce policy**: Reject unrecognized `policyType` values. If the context bit is not SENDER, or `policyType == 0x00`, skip this step. For `policyType == 0x01`, for each entry in `calls`, SLOAD `owner_policy(sender, ownerId, call.to)` and require the returned commitment to be non-zero. The commitment value is opaque to the protocol.
...
rlp([
  ...
  initial_owners      // Array of [verifier, ownerId, scope, policyType, policies] tuples
])

policies = [[callTo, commitment], ...]   // empty unless policyType == 0x01
```

Initial owners are registered with their specified scope and `policyType`. Entries in `policies` populate `owner_policy(sender, ownerId, callTo)` for each new owner. `policies` MUST be empty unless `policyType == 0x01`. Wallet initialization code can lock the account via `calls` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
policies_root_i = keccak256(
    callTo_0 || commitment_0 || ... || callTo_m || commitment_m   // policies_i sorted by callTo; bytes32(0) if empty
)

owners_commitment = keccak256(
    ownerId_0 || verifier_0 || scope_0 || policyType_0 || policies_root_0 ||
    ...
    ownerId_n || verifier_n || scope_n || policyType_n || policies_root_n
)
...
The per-owner contribution is `ownerId || verifier || scope || policyType || policies_root` (86 bytes). Sorting owners by `ownerId` and policies by `callTo` makes the commitment order-independent.
...
1. Parse `[0x00, user_salt, code, initial_owners]` where each entry is `[verifier, ownerId, scope, policyType, policies]`
2. Reject if any duplicate `ownerId` values exist
3. Reject if `code` is empty
4. Reject any owner with unrecognized `policyType`, non-empty `policies` when `policyType != 0x01`, a policy entry with `commitment == bytes32(0)`, or duplicate `callTo` within a single owner's `policies`
5. Sort owners by `ownerId` and each owner's `policies` by `callTo`
6. Compute `owners_commitment` per [Address Derivation](#address-derivation)
7. Compute `effective_salt = keccak256(user_salt || owners_commitment)`
...
11. Require `code_size(sender) == 0` (account not yet deployed)
12. Validate `sender_auth` against one of `initial_owners` (ownerId resolved from auth must match an entry's ownerId and the auth verifier must match that entry's verifier)
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, policyType, policies])` where `policies = [[callTo, commitment], ...]` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, and `policyType`, and writes each policy entry to `owner_policy(account, ownerId, callTo)`. `policies` MUST be empty unless `policyType == 0x01`. |
| `0x02` | `revokeOwner` | `rlp([])` | Revoke an existing owner. Deletes `owner_config` for non-EOA owners; for the implicit EOA owner (`ownerId == bytes32(bytes20(account))`), overwrites with `verifier = REVOKED_VERIFIER` (`type(uint160).max`) to prevent implicit re-authorization. Per-call policy entries are not enumerated; they become unreachable while `owner_config` is empty/revoked. |
| `0x03` | `setPolicy` | `rlp([callTo, commitment])` | Write `owner_policy(account, ownerId, callTo) = commitment`. `commitment == bytes32(0)` clears the entry. The owner referenced by `ownerId` MUST already exist with `policyType == 0x01`. |
...
```
TYPEHASH = keccak256("SignedOwnerChanges(address account,uint64 chainId,uint64 sequence,OwnerChange[] ownerChanges)OwnerChange(uint8 changeType,bytes32 ownerId,bytes data)")

ownerChangeHashes = [keccak256(abi.encode(changeType, ownerId, keccak256(data))) for each ownerChange]
...
Both paths share the same signed owner changes and `change_sequence` counters. `applySignedOwnerChanges()` parses the verifier address from `auth`, calls the verifier to get the `ownerId`, and checks `owner_config`. `authorizeOwner` writes `owner_config` and any provided policy entries; `revokeOwner` clears `owner_config`; `setPolicy` writes a single `owner_policy` entry. Anyone can call these functions; authorization comes from the signed operation, not the caller. All owner modification paths are blocked when the account is locked (see [Account Lock](#account-lock)).
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, policyType, policies]` tuple, write `owner_config` (verifier address, scope byte, and `policyType`) and, for each entry in `policies`, write `owner_policy(sender, ownerId, callTo) = commitment`. Initialize lock state to safe defaults: `locked = false`, `unlock_delay = 0`, `unlocks_at = 0`.
...
If the wallet needs the verifier address or scope, it calls `getOwnerConfig(account, ownerId)` on the Account Configuration Contract; per-call policy commitments are read via `getPolicyCommitment(account, ownerId, callTo)`.
...
The protocol reads everything it needs for verifier binding, scope checking, and policy selection in one SLOAD on `owner_config`. Per-call policy commitments are kept in a separate map (`owner_policy`) so owners with `policyType == 0x00` pay nothing for the feature; owners with `policyType == 0x01` pay one SLOAD per `call` only when they authenticate as SENDER (see [Why Per-Call Policies?](#why-per-call-policies)).

### Why Per-Call Policies?

Session keys and gas-payer hot wallets often need narrow authority — only this token, only this swap router, only this exchange. Encoding that as a per-call mapping keeps protocol validation simple (one SLOAD per `call`, only when the owner opts in via `policyType == 0x01`) while letting policy contracts express arbitrary semantics off-protocol.

The commitment is an opaque `bytes32`. Policy contracts called through `calls` retrieve it from the Account Configuration Contract together with the authenticated `ownerId` (Transaction Context precompile) and decide what it means: a hash of allowed-method selectors, a hash of spend-limit data initialised on first use, a Merkle root, a tag-and-hash, etc. The protocol enforces *that* a target is authorised; the policy contract enforces *what* the call may do.

This subsumes simpler patterns. "Calls must go to the account itself" is a single entry with `callTo == sender`; "calls must go to one address X" is a single entry for `X`. It also extends to richer use cases such as native token spend limits, where a `transferWithAuthorization(policyData, amount, to)` call proves `keccak256(policyData)` matches the stored commitment before the policy updates spend state.

A 1-byte `policyType` selector rather than a scope-mask bit because the storage layout is permanent: a byte leaves space for future modes that don't reduce to "allowlist by `to` with opaque commitment" — for example, denylist semantics, alternative keying like `(to, selector)`, or cheap fixed-target modes — without burning scope bits or repurposing reserved bytes. Today only `0x01` is defined; unrecognized values are rejected so future ERCs can lift specific values cleanly.
...
- **Simplicity**: One primary storage slot per owner (`owner_config`). No variable-length public key encoding, no multi-slot public key reads, no length fields. Registration is a single SSTORE in the common case, with one extra SSTORE per per-call policy entry when an owner uses `policyType == 0x01`.
...
interface IAccountConfiguration {
    ...
    struct OwnerConfig {
        ...
        uint8 policyType;  // 0x00 = none, 0x01 = per-call mapping
    }
    ...
    struct Owner {
        bytes32 ownerId;
        OwnerConfig config;
        PolicyEntry[] policies;
    }

    struct OwnerChange {
        uint8 changeType;  // 0x01 = authorizeOwner, 0x02 = revokeOwner, 0x03 = setPolicy
        bytes32 ownerId;
    ...
    event PolicySet(address indexed account, bytes32 indexed ownerId, address indexed callTo, bytes32 commitment);
    ...
    function getPolicyCommitment(address account, bytes32 ownerId, address callTo) external view returns (bytes32 commitment);
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes, `owner_policy(sender, ownerId, call.to)` changes for any `call.to` referenced by the transaction (only when `policyType == 0x01`), and nonce consumption. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern. Policy contracts that read the commitment during execution MUST treat it as part of their own authorization state.
...
**Owner Scope and Policy**: Protocol-enforced after verifier execution — a verifier cannot bypass scope checking or the configured call policy.
...
**Account Creation Security**: `initial_owners` (verifier + ownerId + scope + policyType + policies tuples) are salt-committed, preventing front-running of owner assignment or initial policy bindings. Wallet bytecode should be inert when uninitialized as it can be permissionlessly deployed.
