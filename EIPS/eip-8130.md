...
| reserved | 27–31 | MUST be zero on write (native and EVM paths); reject nonzero. Implicit version discriminator: new-format configs with nonzero reserved fail to apply through legacy deployments rather than applying with restrictions dropped |

When `scope & POLICY_SENDER != 0`, the actor's policy is held in two additional slots:

```
policy_commitment(account, actorId) → bytes32   // set when POLICY_SENDER is set
policy_manager(account, actorId)    → address   // set when POLICY_SENDER is set
```
...
**Implicit EOA authorization**: A native secp256k1 signature recovering to the account authenticates as the self-actor whenever the account's `DEFAULT_EOA_REVOKED` flag is not set, resolved from the inline default-EOA config. For a fresh account that inline config is all-zero, `scope` is `0x00` and `expiry` is `0` (full permissions). This allows every existing EOA to send AA transactions immediately without prior registration. When this path applies, the protocol authenticates using native ecrecover rather than calling an external authenticator contract.
...
The scope byte in `actor_config` is a permission bitmask of **grants**. A value of `0x00` means unrestricted (all contexts). Any non-zero value grants only the contexts whose bits are set. Reads are fail-closed: a context is authorized only when `scope == 0x00` or the corresponding grant bit is present. Unknown bits grant nothing. All future scope bits MUST be pure grants.

| Bit | Value | Name | Context |
|-----|-------|------|---------|
| 0 | `0x01` | SIGNATURE | ERC-1271 via `verifySignature()` |
| 1 | `0x02` | SENDER | Ungated initiation: `sender_auth`; may originate transactions to any `call.to` |
| 2 | `0x04` | PAYER | Gas payment: `payer_auth` (sponsored) and the sender's own gas (self-pay) |
| 3 | `0x08` | CONFIG | Config change `auth` |
| 4 | `0x10` | POLICY_SENDER | Gated initiation: `sender_auth`; may originate transactions only to the actor's `policy_manager` (see [Actor Policies](#actor-policies)) |
| 5 | `0x20` | SEQUENCE | May consume the account's sequential nonce for sender-context transactions. Without this bit, a restricted actor (`scope != 0x00`) is nonce-free only (see [Nonce Storage](#nonce-storage)) |
| 6–7 | `0x40`, `0x80` | (spare) | Reserved for future pure grants |

**Initiation modes.** `SENDER` and `POLICY_SENDER` are mutually exclusive at **use** time, not at write time. The protocol (native validation / call execution) MUST reject a `sender_auth` whose resolved `scope` sets both, or sets `POLICY_SENDER` together with `SIGNATURE` or `CONFIG`. `POLICY_SENDER | PAYER` is permitted: the gate constrains calls, not gas, so a session key may self-pay within the account's ETH balance and its `expiry`. `POLICY_SENDER | SEQUENCE` is also permitted: a gated actor MAY additionally be granted `SEQUENCE` to consume the account's shared sequential nonce when strict ordering against the account's other sequenced transactions is required (an "ordered gated actor"); without `SEQUENCE` a `POLICY_SENDER` actor is nonce-free only. An invalid combo stored on-chain is inert (fails authorization) rather than unwritable, so `authorizeActor` does not enumerate combinations.

**Verbatim reporting.** `getActorConfig` and `authenticateActor` MUST return the stored scope byte unmodified — no normalization or masking of unrecognized bits.

Scope authorization is applied only after an authenticator authenticates the signature and returns an `actorId`: the protocol loads that actor's `scope` and checks it against the context being authorized. For `sender_auth`, require `scope == 0x00 || (scope & (SENDER | POLICY_SENDER)) != 0`, then apply the exclusivity rules above. For `payer_auth` / self-pay require PAYER (or unrestricted); for `verifySignature()` require SIGNATURE (or unrestricted); for config change `auth` require CONFIG (or unrestricted). For a sequenced (non-nonce-free) transaction, additionally require `scope == 0x00 || (scope & SEQUENCE) != 0` on the resolved sender actor; see [Nonce Storage](#nonce-storage).
...
`POLICY_SENDER` selects whether an actor is gated. When set at authorize time, `policyData` MUST be `manager` (20 bytes) ‖ `commitment` (32 bytes) with both nonzero, and those values are written to `policy_manager` / `policy_commitment`. When unset, `policyData` MUST be empty and any prior policy slots are cleared. Policy vocabulary lives in the commitment and manager logic, not in a protocol sub-type byte.

| `POLICY_SENDER` | Gate: `call.to` MUST equal | `policyData` at authorize |
|----------------|----------------------------|---------------------------|
| unset | (no gate) | empty |
| set | the actor's `manager` | `manager` (20) ‖ `commitment` (32), both nonzero |
...
**Scope.** `POLICY_SENDER` is the gated initiation grant. It MUST NOT be combined with `SENDER`, `SIGNATURE`, or `CONFIG` at use time (see [Actor Scope](#actor-scope)). It MAY combine with `PAYER` so a session key can self-pay gas; that gas spend is not policy-gated and is bounded by balance and `expiry`. It MAY also combine with `SEQUENCE` when the key's transactions must be strictly ordered against the account's other sequenced traffic; the session-key default, and the recommended choice absent a specific ordering requirement, is `POLICY_SENDER` **without** `SEQUENCE`, i.e. nonceless, so parallel session keys never contend with the owner's sequence or each other (see [Nonce Storage](#nonce-storage)). `SIGNATURE` is excluded so permit-style ERC-1271 bypasses are structurally closed; session-key signing SHOULD use an account-level approved-hash / approve-typed-data pattern (Safe `approveHash` precedent) rather than granting `SIGNATURE` on the policy key.

**Reference flow** (non-normative). One construction of a session-key policy:

1. **Authorize.** The account authorizes the key with `POLICY_SENDER` (optionally `| PAYER`, and `| SEQUENCE` only if ordering is required), the `manager`, and a `commitment`. The key's call authority is only the gate to its `manager`.
...
**Example** (non-normative). A subscription session key limited to **5 USDC per 30-day period** with a two-target allowlist. Authorized with `POLICY_SENDER` (nonceless, without `SEQUENCE`), and without `PAYER` if a sponsor pays gas. Over-budget transfers, wrong targets, expiry, or revoke all fail.
...
Restrictions the Account Configuration contract must enforce are complete at deployment: the signer's CONFIG grant, reserved-bytes-zero (also the packing-format discriminator), and well-formed `policyData` when `POLICY_SENDER` is set. Grants can grow because reads fail closed on unknown bits, so new pure-grant bits (e.g. `SEQUENCE`, or the remaining spare bits) are additive without rewriting historical configs or the contract itself. Signing semantics (e.g. approved typed data for session keys) evolve at the account layer, not in this contract.

#### Nonce Storage

Nonce state is managed by a precompile at `NONCE_MANAGER_ADDRESS`. The protocol reads and increments a single sequential nonce counter per account directly during AA transaction processing; the precompile exposes a read-only `getNonce()` interface to the EVM.

The transaction carries one nonce field, `nonce` (`uint64`): for a sequenced transaction it is the account's expected next sequence number; for a nonce-free transaction it is the sentinel `NONCE_FREE` (`2^64 - 1`).

| `nonce` | Mode | Description |
|---------|------|-------------|
| `0` through `NONCE_FREE - 1` | Sequenced | Must equal the account's current sequential counter; incremented after inclusion regardless of execution outcome |
| `NONCE_FREE` | Nonce-free | No nonce state read or incremented; replay protection via `expiry` and `replay_id` |

A restricted actor (`scope != 0x00`) may only send a sequenced transaction (`nonce != NONCE_FREE`) when its scope grants `SEQUENCE`; otherwise it MUST use `NONCE_FREE` (see [Actor Scope](#actor-scope)). An unrestricted actor (`scope == 0x00`) may use either mode freely.

##### Nonce-Free Mode (`NONCE_FREE`)

When `nonce == NONCE_FREE`, the protocol does not read or increment the sequential counter. Replay protection relies on `expiry`, which MUST be non-zero. The `nonce_cost` for this mode (see [Intrinsic Gas](#intrinsic-gas)) covers `replay_id` deduplication state, which is distinct from the sequential counter and is maintained to detect duplicate transactions within the expiry window.

Nodes SHOULD reject `NONCE_FREE` transactions from the mempool if `expiry` exceeds a short window (e.g., 10 seconds from current time). Replay protection and mempool replacement are handled by the **replay identifier** defined below.

###### Replay Identifier

Every AA transaction has a **signature- and fee-invariant** identifier, `replay_id`, used for nonce-free deduplication and for mempool fee-bump replacement (both sequenced and nonce-free). Unlike the [EIP-2718](./eip-2718.md) transaction hash, `replay_id` excludes the fee fields (`max_priority_fee_per_gas`, `max_fee_per_gas`, `gas_limit`) and both authorization blobs (`sender_auth`, `payer_auth`), so a fee bump or a re-signed authorization does not change the transaction's identity, while it still commits to everything that determines the transaction's *effect*:

```
REPLAY_ID_TYPE = 0x7B01

replay_id = keccak256(REPLAY_ID_TYPE || rlp([
  chain_id, resolved_sender, nonce, expiry,
  account_changes, calls, metadata,
  payer
]))
```

`resolved_sender` is the sender address recovered via ecrecover (EOA path, where `sender` is empty in the wire format) or taken directly from the `sender` field (configured-actor path); binding it keeps the identifier unique per sender on the EOA path, where two distinct EOAs can sign identical transaction bodies. For a nonce-free transaction, `nonce` is the `NONCE_FREE` sentinel. `metadata` is included as attribution/annotation entropy, so transactions differing only in that field are treated as distinct. `payer` is the payer *address* field, not `payer_auth`: changing who pays is a change in economic effect, so it produces a new `replay_id`, and a sponsored transaction is never the "same" transaction as an otherwise-identical self-paid one.
...
**Mempool replacement** (normative):

- Two pending transactions from the same `sender` with the same `replay_id` are **replacement candidates** for each other, regardless of nonce mode.
- Nodes rank replacement candidates by fee and MUST require a minimum bump (e.g. ≥10% higher `max_priority_fee_per_gas`) to accept a replacement, mirroring standard fee-bump replacement policy.
- A replacement MUST be a fully valid transaction in its own right, including a valid `payer_auth` where applicable: since `payer_auth` commits to the fee fields via the payer's signature, a bumped-fee replacement of a sponsored transaction needs a fresh `payer_auth`. A sender MAY replace a stalled sponsored transaction with a self-paid one, but changing `payer` produces a **different** `replay_id`, so this is a new logical transaction, not a same-slot replacement.
- Consensus is unchanged from the nonce-free-only rule it generalizes: block builders MUST NOT include two transactions with the same `replay_id` in the same block.
...
// Per-actor. For imported actors scope and expiry are 0 and policyData is empty:
...
AA_TX_TYPE || rlp([
  ...
  nonce,              // uint64: expected sequence number | NONCE_FREE (2^64 - 1) for nonce-free mode
...
| `nonce` | `uint64`. **Sequenced**: must match the account's current sequential counter; incremented after inclusion regardless of execution outcome. Requires `SEQUENCE` scope on a restricted actor. **Nonce-free**: the sentinel `NONCE_FREE` (`2^64 - 1`); the sequential counter is neither read nor incremented. See [Nonce Storage](#nonce-storage). |
| `expiry` | Unix timestamp (seconds since epoch). Transaction invalid when `block.timestamp > expiry`. A value of `0` means no expiry. Must be non-zero when `nonce == NONCE_FREE`. |
...
| `nonce_cost` | `NONCE_FREE`: 14,000 gas (`replay_id` deduplication state: 2 cold SLOADs + 1 warm SLOAD + 3 warm SSTORE resets). Sequenced: 5,000 gas (single counter update: cold SLOAD + warm SSTORE reset). The gap between the two figures is a gas-schedule artifact of the underlying storage operations, not a fundamental cost difference between the two modes (see [Why Not 2D Nonces?](#why-not-2d-nonces)) |
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Authorize a new actor. Writes `actor_config` with `authenticator`, `scope`, and `expiry` (`0` = no expiry) verbatim (unknown scope bits stored as-is). If `POLICY_SENDER` is set, requires `policyData = manager ‖ commitment` (both nonzero, 52 bytes) and writes those slots; otherwise requires empty `policyData` and clears policy slots. Rejects nonzero reserved bytes. Does **not** enforce scope-combination exclusivity (that is use-time / protocol). Emits `ActorAuthorized`. |
...
A locked account's total invalidation surface is a single sequential nonce counter plus the expiry-bounded `replay_id` set (see [Nonce Storage](#nonce-storage)): `actor_config` is frozen, so a pending transaction can only be invalidated by consumption of that one counter (sequenced) or by expiry/`replay_id` deduplication (nonce-free). This bounded surface is what lets nodes cache actor state and apply higher rate limits for locked accounts (see below).
...
7. Verify nonce, payer ETH balance, and expiry:
   - **Sequenced** (`nonce != NONCE_FREE`): require `scope == 0x00 || (scope & SEQUENCE) != 0` on the resolved sender actor, and require `nonce == current_sequence(sender)`.
   - **Nonce-free** (`nonce == NONCE_FREE`): skip the sequential-counter check, require non-zero `expiry`, and nodes SHOULD reject if `expiry` exceeds a short window (e.g., 10 seconds). Deduplicate by the [Replay Identifier](#replay-identifier) (`replay_id`), not the full transaction hash.
8. Mempool threshold: gas payer's pending count below node-configured limits.
9. Replacement: if a pending transaction from the same `sender` shares this transaction's `replay_id`, treat it as a fee-bump replacement per the [Mempool replacement](#replay-identifier) rules rather than a distinct transaction.
...
3. If `nonce != NONCE_FREE`, increment the account's sequential nonce counter in Nonce Manager storage for `sender`. If `nonce == NONCE_FREE`, skip (nonce-free mode).
...
**`eth_getTransactionCount`**: Standard behavior (no protocol extension needed): returns the account's single sequential nonce counter, read from the Nonce Manager precompile at `NONCE_MANAGER_ADDRESS`.
...
| `NONCE_FREE` | `2^64 - 1` | Nonce-free mode sentinel for the `nonce` field (expiry- and `replay_id`-based replay protection) |
| `REPLAY_ID_TYPE` | `0x7B01` | Domain prefix for `replay_id` computation |
...
Session keys often need narrow authority: only this token, only this much per day, only this action. `POLICY_SENDER` makes gated initiation a first-class grant, distinct from ungated `SENDER`: the key may originate transactions, but only to a single call target, bound to a signed opaque **commitment**. The protocol's only policy responsibility is the single-target gate plus storing the commitment.

Because Account Configuration is a single immutable deployment, policy vocabulary is not encoded as protocol sub-type bytes. Excluding `SIGNATURE` on policy keys structurally closes permit-style ERC-1271 bypasses; session-key signing belongs at the account layer (approved-hash / approve-typed-data). Allowing `POLICY_SENDER | PAYER` lets a session key self-pay a small ETH budget without escaping the call gate.

### Why the SEQUENCE Grant?

Parallel session keys should not contend for the owner's sequential nonce. Rather than reserving each key its own nonce channel, restricted actors (`scope != 0x00`) are nonce-free by default: without `SEQUENCE`, a key's transactions carry `nonce == NONCE_FREE` and never touch the shared counter, so any number of session keys can be used concurrently without coordinating a sequence number. `SEQUENCE` is the escape hatch for the actor that does need the account's shared ordering guarantee, for example a gated policy actor whose transactions must be strictly ordered relative to the owner's own sequenced transactions (`POLICY_SENDER | SEQUENCE`, an "ordered gated actor"). This gets the isolation that separate nonce lanes provided, without a second dimension of protocol nonce state: unrestricted actors and `SEQUENCE`-scoped actors share the account's single counter, and everyone else is nonce-free.
...
An earlier design used a two-dimensional nonce (`nonce_key`, `nonce_sequence`) so wallets could open arbitrarily many parallel sequential channels per account. In practice the jobs that motivated additional channels are better handled by mechanisms this proposal already has:

- **Phased batching** (independent workflows within one transaction that shouldn't block each other): [Call Phases](#call-phases) already give ordered, independently-committing groups of calls without any extra nonce state.
- **Nonce-free + replacement** (fire-and-forget or latency-sensitive workflows that don't want to coordinate a sequence number): nonce-free mode plus [`replay_id`](#replay-identifier)-based mempool replacement covers submission, deduplication, and fee bumping without a dedicated channel.
- **Sub-accounts** (workloads that want their own address-level namespace, not just their own nonce channel): a separate account (via a create entry) or a [delegate actor](#delegate-authenticator) gives full isolation, which a shared-account nonce channel could not.
- **Dormant, pre-authorized actors** (keys installed in advance that may never transact, or transact rarely): these are exactly `POLICY_SENDER`/restricted actors without `SEQUENCE`, i.e. nonce-free by default, so provisioning them costs nothing until used and never contends with any channel.

With those cases covered, a second nonce dimension adds a permanently-growing, per-`(account, key)` storage dimension for a benefit already available elsewhere. The single sequential counter plus nonce-free mode keeps the same coverage with one persistent nonce slot per account. The `nonce_cost` gap between sequenced (5,000 gas) and nonce-free (14,000 gas) transactions (see [Intrinsic Gas](#intrinsic-gas)) is a gas-schedule artifact of the specific storage operations each mode performs under current EVM pricing, not evidence that parallel channels would be cheaper: nonce-free's cost is dominated by `replay_id` dedup-state writes, not by the absence of a counter increment.
...
interface IAccountConfiguration {
    ...
    struct ActorConfig {
        ...
        uint8 scope;        // grants bitmask; 0x00 = unrestricted; 0x10 = POLICY_SENDER; 0x20 = SEQUENCE
    ...
    struct Actor {
        bytes32 actorId;
        ActorConfig config;
        bytes policyData;  // empty unless POLICY_SENDER set; then manager[20] || commitment[32]
    }
    ...
    //   authenticator(20) || scope(1) || expiry(6) — 27 bytes —
    //   and, only when scope & POLICY_SENDER != 0, manager(20) || commitment(32) (79 bytes total).
    ...
    // Aggregate: (manager, commitment); both zero if ungated.
...
Read-only. The protocol manages nonce storage directly; there are no state-modifying functions. Gas is charged as a base cost plus a cold SLOAD (2,100 gas) for the first read of a given `account`'s nonce slot in the transaction, or a warm SLOAD (100 gas) for subsequent reads of the same slot, consistent with [EIP-2929](./eip-2929.md) access rules.

## Security Considerations

**Validation Surface**: For canonical authenticators, invalidators are `actor_config` changes and nonce consumption.

**Replay Protection**: Transactions include `chain_id`, a sequential `nonce` (or the `NONCE_FREE` sentinel for nonce-free mode), and `expiry`. For nonce-free transactions, replay protection relies on short-lived `expiry` and deduplication by the [Replay Identifier](#replay-identifier) (`replay_id`), which also underlies fee-bump mempool replacement for both sequenced and nonce-free transactions (see [Nonce Storage](#nonce-storage)). The mempool enforces a tight expiry window (e.g., 10-30 seconds) to bound the window. Block builders MUST NOT include two transactions with the same `replay_id`.

The full transaction hash MUST NOT be used for nonce-free deduplication or replacement matching. The transaction hash commits to the fee fields (`max_priority_fee_per_gas`, `max_fee_per_gas`, `gas_limit`) and to both authorization blobs (`sender_auth`, `payer_auth`), whereas `replay_id` excludes all of them and instead commits to the transaction's *effects* (`account_changes`, `calls`, `metadata`, `expiry`, `payer`). Keying on the transaction hash would treat every fee bump and every re-signature as an unrelated new transaction rather than a replacement of the same logical one:

- **Fee bumps**: a fee-bumped resubmission changes `max_priority_fee_per_gas` / `max_fee_per_gas` and, for a sponsored transaction, requires a fresh `payer_auth` over the new fees; both change the transaction hash but not `replay_id`, which is exactly what lets a bump be treated as a replacement rather than a distinct transaction.
...
All of these resolve to the same `replay_id`, so deduplicating and replacing on it collapses them to a single mempool slot and a single includable transaction. Because `expiry` is part of `replay_id`, a transaction's expiry cannot be extended by "replacing" it: changing `expiry` produces a different `replay_id`, so it is a new logical transaction rather than a same-slot fee bump of the old one. Similarly, because `payer` is part of `replay_id`, switching a sponsored transaction to self-pay (or vice versa) is a new logical transaction, not a replacement, since it changes who bears the economic cost.

**Actor Scope and Policy**: Scope grants are protocol-enforced after authenticator execution during validation (fail closed). The policy gate is protocol-enforced during execution (`ActorPolicyViolation`). The gate constrains calls, not gas: `POLICY_SENDER | PAYER` may self-pay, with drain bounded by balance and `expiry`; sponsored gas is unchanged. `POLICY_SENDER` excluding `SIGNATURE` structurally closes permit-style bypasses; session-key signing SHOULD use an account-level approved-hash / approve-typed-data pattern (Safe `approveHash` precedent). Illegal `POLICY_SENDER` combinations with `SENDER` / `SIGNATURE` / `CONFIG` are rejected at use time (see [Actor Scope](#actor-scope)).
