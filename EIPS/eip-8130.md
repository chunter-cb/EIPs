...
Each owner occupies a single `owner_config` slot containing the verifier address, scope byte, and policy type. When `policyType == 0x02`, the required policy system address is stored in `owner_policy_target(account, ownerId)` and an opaque policy data commitment is stored in `owner_policy_commitment(account, ownerId)`. Non-EOA owners are revoked by deleting the `owner_config` slot. The implicit EOA owner (`ownerId == bytes32(bytes20(account))`) is revoked by overwriting the slot with `verifier = REVOKED_VERIFIER` (`type(uint160).max`), making it distinguishable from an empty (implicitly authorized) slot.

| Field | Bytes | Description |
|-------|-------|-------------|
| `verifier` | 0–19 | Verifier contract address |
| `scope` | 20 | Permission bitmask (`0x00` = unrestricted) |
| `policyType` | 21 | Policy selector (`0x00` = none; see [Policy Types](#policy-types)) |
| reserved | 22–31 | Reserved for future use (must be zero) |

**Implicit EOA authorization**: An unregistered owner (`owner_config` slot is empty) is implicitly authorized if `ownerId == bytes32(bytes20(account))`. The empty slot's scope byte and policy type are `0x00`, granting unrestricted permissions and no call policy by default. This allows every existing EOA to send AA transactions immediately without prior registration. When the implicit rule applies, the protocol verifies using native ecrecover rather than calling an external verifier contract. The implicit authorization is revoked by writing `REVOKED_VERIFIER` (`type(uint160).max`) to the verifier field, making the slot non-empty and blocking re-authorization. The EOA owner can also be explicitly registered with `ECRECOVER_VERIFIER` (`address(1)`) to set a custom scope or policy while retaining native ecrecover verification.
...
The `policyType` field selects an optional protocol-enforced call policy. It is a 1-byte `uint8`.

| `policyType` | Name | Storage | Description |
|--------------|------|---------|-------------|
| `0x00` | none | policy companion slots MUST be zero | No call policy. |
| `0x01` | account-only | policy companion slots MUST be zero | Every entry in the transaction's `calls` array MUST have `to == sender`. |
| `0x02` | policy-system | `owner_policy_target(account, ownerId)` stores the required policy system address; `owner_policy_commitment(account, ownerId)` stores an opaque policy data commitment | Every entry in the transaction's `calls` array MUST have `to == owner_policy_target(account, ownerId)`. |
| `0x03`–`0xFF` | reserved | future use | Reserved for future policy types. |

Policy enforcement applies only when the owner authenticates under SENDER scope. PAYER, SIGNATURE, and CONFIG authentication ignore `policyType`.

For `policyType == 0x02`, the protocol only enforces that all SENDER calls route to the required policy system address. The `policyCommitment` is opaque to the protocol. The policy system can read the authenticated `ownerId` from the Transaction Context precompile, fetch `owner_policy_commitment(account, ownerId)` from the Account Configuration Contract, verify policy data against the commitment, and perform application-specific inspection or tracking before executing downstream calls. For example, a policy system can bind a session owner to token-specific spend-limit data and require native token calls such as `transferWithAuthorization(policyData, amount, to)` to prove that the supplied policy data hashes to the stored commitment before updating spend state.
...
| `account_changes_cost` | Per applied config change entry: auth verification cost (same model as `sender_auth_cost`) + storage write costs for each mutated owner slot (`owner_config`, and policy companion slots when set or cleared). Per applied delegation entry: code deposit cost (200 × 23 bytes for the delegation indicator). Per skipped config change entry (already applied): 2,100 (SLOAD to check sequence). 0 if no config change or delegation entries in `account_changes` |
...
6. **Enforce policy**: If the context bit is not SENDER, skip this step. Otherwise read `policyType` from `owner_config` (or `0x00` for the implicit case). For `policyType == 0x00`, no additional check is required. For `policyType == 0x01`, require every entry in the transaction's `calls` array to have `to == sender`. For `policyType == 0x02`, SLOAD `owner_policy_target(sender, ownerId)`, require it to be non-zero, and require every entry in the transaction's `calls` array to have `to == policyTarget`. Reject unrecognized `policyType` values. The protocol does not read or interpret `owner_policy_commitment` during validation.
...
rlp([
  ...
  initial_owners      // Array of [verifier, ownerId, scope, policyType, policyTarget, policyCommitment] tuples
])
```

Initial owners are registered with their specified scope and policy. `policyTarget` and `policyCommitment` are only used when `policyType == 0x02`; otherwise both MUST be zero. Wallet initialization code can lock the account via `calls` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
owners_commitment = keccak256(
    ownerId_0 || verifier_0 || scope_0 || policyType_0 || policyTarget_0 || policyCommitment_0 ||
    ownerId_1 || verifier_1 || scope_1 || policyType_1 || policyTarget_1 || policyCommitment_1 ||
    ...
    ownerId_n || verifier_n || scope_n || policyType_n || policyTarget_n || policyCommitment_n
)
...
The `owners_commitment` uses `ownerId || verifier || scope || policyType || policyTarget || policyCommitment` (106 bytes) per owner — consistent with how the Account Configuration Contract identifies and configures owners.
...
1. Parse `[0x00, user_salt, code, initial_owners]` where each entry is `[verifier, ownerId, scope, policyType, policyTarget, policyCommitment]`
2. Reject if any duplicate `ownerId` values exist
3. Reject if `code` is empty
4. Reject if any entry has an unrecognized `policyType`, or if `policyType != 0x02` with non-zero `policyTarget` or non-zero `policyCommitment`, or if `policyType == 0x02` with `policyTarget == address(0)` or `policyCommitment == bytes32(0)`
5. Sort by ownerId: `sorted_owners = sort(initial_owners, by: ownerId)`
6. Compute `owners_commitment = keccak256(ownerId_0 || verifier_0 || scope_0 || policyType_0 || policyTarget_0 || policyCommitment_0 || ... || ownerId_n || verifier_n || scope_n || policyType_n || policyTarget_n || policyCommitment_n)`
...
owner_change = rlp([
  ...
  policyType,           // uint8: policy selector (authorizeOwner only, 0x00 = none)
  policyTarget,         // address: required policy system when policyType == 0x02, else zero
  policyCommitment      // bytes32: opaque policy data commitment when policyType == 0x02, else zero
])
```
...
|-------------|------|-------------|-------------|
| `0x01` | `authorizeOwner` | Authorize a new owner with scope and policy | `verifier`, `ownerId`, `scope`, `policyType`, `policyTarget`, `policyCommitment` |
...
```
TYPEHASH = keccak256("SignedOwnerChanges(address account,uint64 chainId,uint64 sequence,OwnerChange[] ownerChanges)OwnerChange(uint8 changeType,address verifier,bytes32 ownerId,uint8 scope,uint8 policyType,address policyTarget,bytes32 policyCommitment)")

ownerChangeHashes = [keccak256(abi.encode(changeType, verifier, ownerId, scope, policyType, policyTarget, policyCommitment)) for each ownerChange]
...
Both paths share the same signed owner changes and `change_sequence` counters. `applySignedOwnerChanges()` parses the verifier address from `auth`, calls the verifier to get the `ownerId`, and checks `owner_config`. Authorizing or revoking an owner updates `owner_config` and, when `policyType == 0x02`, the policy companion slots. Anyone can call these functions; authorization comes from the signed operation, not the caller. All owner modification paths are blocked when the account is locked (see [Account Lock](#account-lock)).
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, policyType, policyTarget, policyCommitment]` tuple, write `owner_config` (verifier address, scope byte, and policy type) and, when `policyType == 0x02`, write `owner_policy_target` and `owner_policy_commitment`. Initialize lock state to safe defaults: `locked = false`, `unlockDelay = 0`, `unlockRequestedAt = 0`.
...
If the wallet needs the verifier address, scope, or policy, it calls `getOwnerConfig(account, ownerId)` on the Account Configuration Contract.
...
The protocol reads everything it needs for verifier binding, scope checking, and policy selection in one SLOAD. Only the `policy-system` policy pays an additional SLOAD during validation to load its required policy system address. The opaque policy commitment is stored separately for the policy system to read during execution, keeping protocol validation compact while still binding an owner to application-specific policy data.
...
- **Simplicity**: One primary storage slot per owner (`owner_config`). No variable-length public key encoding, no multi-slot public key reads, no length fields. Registration is a single SSTORE in the common case and uses companion slots only when `policyType == 0x02`.
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes, `owner_policy_target` changes when `policyType == 0x02`, and nonce consumption. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern. `owner_policy_commitment` changes do not affect protocol validation directly, but policy systems that read the commitment during execution MUST treat it as part of their own authorization state.
...
**Account Creation Security**: `initial_owners` (verifier + ownerId + scope + policyType + policyTarget + policyCommitment tuples) are salt-committed, preventing front-running of owner assignment. Wallet bytecode should be inert when uninitialized as it can be permissionlessly deployed.
