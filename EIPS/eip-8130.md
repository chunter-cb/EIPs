...
Each owner occupies a single `owner_config` slot containing the verifier address, scope byte, and policy type. When `policyType == 0x02`, the policy target address is stored in a companion slot `owner_policy_target(account, ownerId)`. Non-EOA owners are revoked by deleting the `owner_config` slot. The implicit EOA owner (`ownerId == bytes32(bytes20(account))`) is revoked by overwriting the slot with `verifier = REVOKED_VERIFIER` (`type(uint160).max`), making it distinguishable from an empty (implicitly authorized) slot.
...
|--------------|------|---------|-------------|
| `0x00` | none | `owner_policy_target` MUST be zero | No call policy. |
| `0x01` | account-only | `owner_policy_target` MUST be zero | Every entry in the transaction's `calls` array MUST have `to == sender`. |
| `0x02` | explicit-address | `owner_policy_target(account, ownerId)` stores the target address | Every entry in the transaction's `calls` array MUST have `to == owner_policy_target(account, ownerId)`. |
...
| `account_changes_cost` | Per applied config change entry: auth verification cost (same model as `sender_auth_cost`) + storage write costs for each mutated owner slot (`owner_config`, and `owner_policy_target` when set or cleared). Per applied delegation entry: code deposit cost (200 × 23 bytes for the delegation indicator). Per skipped config change entry (already applied): 2,100 (SLOAD to check sequence). 0 if no config change or delegation entries in `account_changes` |
...
rlp([
  ...
  initial_owners      // Array of [verifier, ownerId, scope, policyType, policyTarget] tuples
])
```

Initial owners are registered with their specified scope and policy. `policyTarget` is only used when `policyType == 0x02`; otherwise it MUST be zero. Wallet initialization code can lock the account via `calls` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
owners_commitment = keccak256(
    ownerId_0 || verifier_0 || scope_0 || policyType_0 || policyTarget_0 ||
    ownerId_1 || verifier_1 || scope_1 || policyType_1 || policyTarget_1 ||
    ...
    ownerId_n || verifier_n || scope_n || policyType_n || policyTarget_n
)
...
The `owners_commitment` uses `ownerId || verifier || scope || policyType || policyTarget` (74 bytes) per owner — consistent with how the Account Configuration Contract identifies and configures owners.
...
1. Parse `[0x00, user_salt, code, initial_owners]` where each entry is `[verifier, ownerId, scope, policyType, policyTarget]`
2. Reject if any duplicate `ownerId` values exist
3. Reject if `code` is empty
4. Reject if any entry has an unrecognized `policyType`, or if `policyType != 0x02` with non-zero `policyTarget`, or if `policyType == 0x02` with `policyTarget == address(0)`
5. Sort by ownerId: `sorted_owners = sort(initial_owners, by: ownerId)`
6. Compute `owners_commitment = keccak256(ownerId_0 || verifier_0 || scope_0 || policyType_0 || policyTarget_0 || ... || ownerId_n || verifier_n || scope_n || policyType_n || policyTarget_n)`
...
owner_change = rlp([
  ...
  policyTarget          // address: explicit policy target when policyType == 0x02, else zero
])
```
...
|-------------|------|-------------|-------------|
| `0x01` | `authorizeOwner` | Authorize a new owner with scope and policy | `verifier`, `ownerId`, `scope`, `policyType`, `policyTarget` |
...
```
TYPEHASH = keccak256("SignedOwnerChanges(address account,uint64 chainId,uint64 sequence,OwnerChange[] ownerChanges)OwnerChange(uint8 changeType,address verifier,bytes32 ownerId,uint8 scope,uint8 policyType,address policyTarget)")

ownerChangeHashes = [keccak256(abi.encode(changeType, verifier, ownerId, scope, policyType, policyTarget)) for each ownerChange]
...
Both paths share the same signed owner changes and `change_sequence` counters. `applySignedOwnerChanges()` parses the verifier address from `auth`, calls the verifier to get the `ownerId`, and checks `owner_config`. Authorizing or revoking an owner updates both `owner_config` and, when `policyType == 0x02`, the companion `owner_policy_target` slot. Anyone can call these functions; authorization comes from the signed operation, not the caller. All owner modification paths are blocked when the account is locked (see [Account Lock](#account-lock)).
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, policyType, policyTarget]` tuple, write `owner_config` (verifier address, scope byte, and policy type) and, when `policyType == 0x02`, write `owner_policy_target`. Initialize lock state to safe defaults: `locked = false`, `unlockDelay = 0`, `unlockRequestedAt = 0`.
...
The protocol reads everything it needs for verifier binding, scope checking, and policy selection in one SLOAD. Only the `explicit-address` policy pays an additional SLOAD to load its target address. This keeps the common case compact while still allowing one arbitrary address-based policy.
...
- **Simplicity**: One primary storage slot per owner (`owner_config`). No variable-length public key encoding, no multi-slot public key reads, no length fields. Registration is a single SSTORE in the common case and a second SSTORE only when `policyType == 0x02`.
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes, `owner_policy_target` changes when `policyType == 0x02`, and nonce consumption. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern.
...
**Account Creation Security**: `initial_owners` (verifier + ownerId + scope + policyType + policyTarget tuples) are salt-committed, preventing front-running of owner assignment. Wallet bytecode should be inert when uninitialized as it can be permissionlessly deployed.
